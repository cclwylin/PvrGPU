#!/usr/bin/env bash
# Build the current generic UBO compiler ABI regression against existing Mesa
# libraries. No installed driver or model binary is changed.
set -euo pipefail
...
executable = File.join(output, 'generic-ubo-abi-test')
...
abort 'UBO compiler ABI test failed' unless system(executable)
puts "UBO compiler unit artifacts: #{output}"
RUBY
