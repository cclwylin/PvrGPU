...
# Usage: bash script/run_mesa_resource_unit.sh [all|blit|clear|ubo|compute|surface]
set -euo pipefail
...
abort 'usage: run_mesa_resource_unit.sh [all|blit|clear|ubo|compute|surface]' unless
  %w[all blit clear ubo compute surface].include?(selected)
