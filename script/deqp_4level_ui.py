...
``docs/dEQP_4level.md``:
  ...
  * the four tiers L1..L4; L1..L3 omit the explicitly catalogued long-running
    families while L4 retains every discovered case;
...
The shared 30-group directory and planning rules live in
``script/deqp_4level_catalog.py``, also used by the command-line level runner.
The tiers select *which* cases run, never what their result should be.
"""
...
class MainWindow(QMainWindow):
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        self.plan_table.setHorizontalHeaderLabels(
            ["#", "組名", "Suite", "N", "L1", "L2", "discovery", "L4-only", "本次選取"]
        )
    ...
    def _update_group_note(self) -> None:
        ...
        tier = self._current_tier()
        pool = self._group_pool(group)
        discovered = len(pool)
        long_excluded = 0 if tier.whole_modules else sum(
            is_long_only_case(case_name) for case_name in pool
        )
        eligible = discovered - long_excluded
        note = (
        ...
        note += f"\n這一層取 {min(group.quota(tier.id), max(eligible, 0))} 條"
        if long_excluded:
            note += f"；另有 {long_excluded} 條長測項只留在 L4"
    ...
    def _refresh_plan_tab(self) -> None:
        ...
        long_by_number = {
            entry.group.number: entry.long_excluded for entry in (plan.groups if plan else [])
        }
        mode = self.mode_combo.currentText()
        tier = self._current_tier()
        ...
        for row, group in enumerate(GROUPS_4LEVEL):
            ...
            long_excluded = long_by_number.get(group.number, 0)
            values = [
                str(group.number),
                group.label,
                ...
                str(long_excluded) if long_excluded else "-",
            ...
            for column, value in enumerate(values):
                ...
                if column == 7 and long_excluded:
                    item.setForeground(QColor(self.theme.warn))
                if column == 8 and chosen:
        ...
        if plan is None:
            ...
            if not plan.tier.whole_modules and plan.long_excluded_count:
                details.append(
                    f"  · {plan.long_excluded_count} 個長測項不進 L1/L2/L3，完整保留在 L4"
                )
    ...
    def _refresh_plan_note(self, plan: RunPlan | None = None) -> None:
        ...
        self.plan_note.setText(
            ...
            f"{format_seconds(plan.estimated_seconds() / max(plan.shards, 1))}。"
            + (
                f"另有 {plan.long_excluded_count} 個長測項只在 L4。"
                if not plan.tier.whole_modules and plan.long_excluded_count
                else ""
            )
        )
    ...
    def build_commands(self, plan: RunPlan) -> str:
        ...
        if plan.tier.whole_modules:
            ...
            long_pattern = "|".join(
                "^" + self._escape(prefix) for prefix in LONG_ONLY_PREFIXES
            )
            add("# L1/L2/L3 排除已知長測項；它們仍完整保留在 L4。")
            per_module: dict[str, list[str]] = {}
            for entry in plan.groups:
                if not entry.selected:
                    if entry.long_excluded:
                        add(
                            f"# 第 {entry.group.number} 組 {entry.group.label}："
                            f"{entry.long_excluded} 條全數留在 L4"
                        )
                    continue
                group = entry.group
                add(
                ...
                raw_target = (
                    f"{tmp}_g{group.number}_unfiltered.txt"
                    if entry.long_excluded
                    else f"{tmp}_g{group.number}_all.txt"
                )
                add(
                ...
                if entry.long_excluded:
                    add(
                        f"grep -vE '{long_pattern}' {raw_target}"
                        f" > {tmp}_g{group.number}_all.txt"
                    )
                eligible_count = entry.discovered - entry.long_excluded
                if entry.quota < eligible_count:
                ...
                "groups": [
                    {
                        ...
                        "long_excluded_l4_only": entry.long_excluded,
                    }
                    for entry in plan.groups
                    if entry.selected or entry.long_excluded
                ],
            }
