...
COUNTER_INFO: Mapping[str, tuple[str, str, str]] = {
    "tcs_invocations": ("TCS invocations", "invocations", "Actual tessellation-control shader lanes."),
    "tcs_alu_instructions": ("TCS ALU", "instructions", "Executed native tessellation-control ALU lane instructions."),
    "tcs_memory_instructions": ("TCS memory", "instructions", "Executed native tessellation-control LD/ST instructions."),
    "tcs_load_instructions": ("TCS loads", "instructions", "Executed native tessellation-control DMA loads."),
    "tcs_store_instructions": ("TCS stores", "instructions", "Executed native tessellation-control DMA stores."),
    "tcs_input_write_bytes": ("TCS input writes", "bytes", "Modeled VS-to-TCS input staging bytes."),
    "tcs_input_read_bytes": ("TCS input reads", "bytes", "Patch-input bytes read by native TCS LD."),
    "tcs_output_write_bytes": ("TCS output writes", "bytes", "Patch-output bytes written by native TCS ST."),
    "tcs_output_read_bytes": ("TCS output reads", "bytes", "Patch-output bytes read by native TCS LD."),
    "tes_alu_instructions": ("TES ALU", "instructions", "Executed native tessellation-evaluation ALU lane instructions."),
    "tes_memory_instructions": ("TES memory", "instructions", "Executed native tessellation-evaluation LD/export instructions."),
    "tes_load_instructions": ("TES loads", "instructions", "Executed native tessellation-evaluation DMA loads."),
    "tes_patch_read_bytes": ("TES patch reads", "bytes", "Patch bytes read by native tessellation-evaluation LD."),
    "tessellation_patches": ("Tessellated patches", "patches", "Patches processed by the fixed tessellator, including culled patches."),
    "tessellation_primitives": ("Tessellation primitives", "primitives", "Actual complete primitives generated by the fixed tessellator."),
    "tessellation_domain_write_bytes": ("Tessellation domain writes", "bytes", "Generated UV bytes written through modeled GPU memory."),
    "tessellation_domain_read_bytes": ("Tessellation domain reads", "bytes", "Generated UV bytes read through modeled memory to supply TES lanes."),
    "tessellation_level_read_bytes": ("Tessellation level reads", "bytes", "TCS outer/inner levels read through modeled memory by the fixed tessellator."),
    ...
    "hs_invocations": ("Hull shader", "patches", "Completed tessellation-control patches (not shader lanes)."),
    "ds_invocations": ("Domain shader", "invocations", "Actual tessellation-evaluation domain-point invocations."),
    ...
    "gs_input_read_bytes": ("GS input reads", "bytes", "Primitive-input bytes read by native geometry LD."),
    "slc_writebacks": (
        "SLC writebacks",
        "lines",
...
class DrawListRecord:
    ...
    geometry: ShaderInstructionStats | None = None
    tessellation_control: ShaderInstructionStats | None = None
    tessellation_evaluation: ShaderInstructionStats | None = None
...
def _drawlist_records(
    ...
    for position, raw in enumerate(raw_drawlists):
        ...
        records.append(
            DrawListRecord(
                ...
                geometry=(
                    _shader_instruction_stats(raw["gs"], f"drawlist_stats[{position}].gs")
                    if "gs" in raw else None
                ),
                tessellation_control=(
                    _shader_instruction_stats(raw["tcs"], f"drawlist_stats[{position}].tcs")
                    if "tcs" in raw else None
                ),
                tessellation_evaluation=(
                    _shader_instruction_stats(raw["tes"], f"drawlist_stats[{position}].tes")
                    if "tes" in raw else None
                ),
            )
        )

        record = records[-1]
        if (record.tessellation_control is None) != (record.tessellation_evaluation is None):
            raise CounterProtocolError("drawlist_stats tessellation stage pair is incomplete")
        for shader in (record.tessellation_control, record.tessellation_evaluation):
            if shader and (shader.program_tex_instructions or shader.executed_tex_instructions):
                raise CounterProtocolError("drawlist_stats tessellation texture instructions are unsupported")
    ...
    aggregate_fields = {
        "tcs_invocations": sum(item.tessellation_control.invocations for item in records if item.tessellation_control),
        "tcs_alu_instructions": sum(item.tessellation_control.executed_alu_instructions for item in records if item.tessellation_control),
        "tcs_memory_instructions": sum(item.tessellation_control.executed_memory_instructions for item in records if item.tessellation_control),
        "ds_invocations": sum(item.tessellation_evaluation.invocations for item in records if item.tessellation_evaluation),
        "tes_alu_instructions": sum(item.tessellation_evaluation.executed_alu_instructions for item in records if item.tessellation_evaluation),
    ...
    if "hs_invocations" in values and "tessellation_patches" in values:
        if values["hs_invocations"] != values["tessellation_patches"]:
            raise CounterProtocolError("completed TCS patch count does not match tessellation_patches")
