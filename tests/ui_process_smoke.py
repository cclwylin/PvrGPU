...
def main() -> int:
    ...
    parser.add_argument(
        "--texture-memory-path", choices=("short", "cached"), default="short"
    )
    ...
    texture_path_index = window.texture_memory_path_combo.findData(
        options.texture_memory_path
    )
    if texture_path_index < 0:
        raise RuntimeError(
            f"{options.texture_memory_path} is missing from texture path selector"
        )
    window.texture_memory_path_combo.setCurrentIndex(texture_path_index)
    effective_cache_bypass = (
        "off"
        if options.backend == "pvrgpu" and options.texture_memory_path == "cached"
        else options.cache_bypass
    )
    ...
    def finished(*_args: object) -> None:
        ...
        process_environment = window.process.processEnvironment()
        cache_control_ok = window.cache_bypass_combo.isEnabled() == (
            options.backend == "pvrgpu"
            and options.texture_memory_path != "cached"
        )
        texture_path_control_ok = (
            window.texture_memory_path_combo.isEnabled()
            == (options.backend == "pvrgpu")
        )
        if options.backend == "llvmpipe":
            ok = (
                ...
                and texture_path_control_ok
                and "--cache-bypass" not in command_log
                and "--texture-memory-path" not in command_log
                and not process_environment.contains("PVRGPU_MODEL_MEMORY_MODE")
                and not process_environment.contains("PVRGPU_TEXTURE_MEMORY_PATH")
            )
        else:
            ...
            if options.case in texture_taps:
                ...
                if options.texture_memory_path == "cached":
                    texture_path_ok = all(
                        ...
                        * taps_per_request
                        ...
                        == record.values.get("tcu_line_accesses", 0)
                        and record.values.get("tcu_cycles", 0)
                        == record.values.get("tcu_line_accesses", 0)
                        and record.values.get("memory_direct_read_bytes", 0) == 0
                    ...
                    texture_path_ok = all(
                        ...
                        == record.values.get("texture_requests", 0)
                        * taps_per_request
                        and record.values.get("tcu_line_accesses", 0) == 0
                        and record.values.get("tcu_read_accesses", 0) == 0
                        and record.values.get("tcu_cycles", 0) == 0
                        and record.values.get("memory_direct_read_bytes", 0) > 0
                        for record in window.records
                    )
            if effective_cache_bypass == "on":
                memory_path_ok = all(
                    record.values.get("slc_line_accesses") == 0
                    and record.values.get("slc_bypassed", 0) > 0
            ...
            ok = (
                ...
                and cache_control_ok
                and texture_path_control_ok
                and f"--cache-bypass {effective_cache_bypass}" in command_log
                and f"--texture-memory-path {options.texture_memory_path}"
                in command_log
                and process_environment.value("PVRGPU_TEXTURE_MEMORY_PATH")
                == options.texture_memory_path
                and process_environment.value("PVRGPU_MODEL_MEMORY_MODE")
                == ("bypass" if effective_cache_bypass == "on" else "cache")
                and memory_path_ok
                and texture_path_ok
            )
        print(
            ...
            f"effective_cache_bypass={effective_cache_bypass}",
            f"texture_memory_path={options.texture_memory_path}",
