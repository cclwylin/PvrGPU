/*
 * Gate-14 non-equal-W Parameter Buffer and fragment-USC proof.
 *
 * A W={1,2,4} triangle carries four independently linked smooth vec4 values
 * scaled by {1/16,3/16,1/4,1/2}; their sum is c. The production event-driven
 * ParameterBuffer builds one shared 1/W plane and sixteen distinct varying/W
 * planes. The exact public
 * varyings_shader_4 fragment PCO then executes all FITRP and left-associated
 * FADD operations, proving perspective-correct linkage rather than affine or
 * duplicated-output shortcuts. FIFO traffic contains only the PipelineState
 * MemoryPool handle.
 */
...
void Check(bool condition, const std::string &message) {
  if (!condition)
    throw std::runtime_error(
        "varyings_shader_four perspective test failed: " + message);
}
...
int sc_main(int, char **) {
  try {
    ...
    state.height = 2;
    state.sequence = 1;
    state.functional_case = FunctionalCase::kVaryingsShaderFour;
    ...
    const float varying_scale[4] = {
        1.0F / 16.0F, 3.0F / 16.0F, 1.0F / 4.0F, 1.0F / 2.0F};
    std::vector<std::uint32_t> raster_outputs(3 * 20, 0);
    for (std::size_t vertex = 0; vertex < 3; ++vertex) {
      for (std::size_t varying = 0; varying < 4; ++varying) {
        for (std::size_t component = 0; component < 4; ++component) {
          raster_outputs[vertex * 20 + 4 + varying * 4 + component] =
    ...
    Check(parameters.size() == 1 &&
              ...
              parameters[0].coefficient_set_count == 17 &&
              coefficients.size() == 17 &&
              result.counters.parameter_coefficient_sets == 17 &&
              result.counters.parameter_write_bytes == 272,
          "one triangle owns one 1/W plus sixteen varying/W sets");
    ...
    for (std::size_t varying = 0; varying < 4; ++varying) {
      ...
      CheckPlane(coefficients[base], FloatBits(0.25F * scale),
                 FloatBits(0.0F),
                 FloatBits(0.0F), "scaled-R/W numerator plane");
    ...
    Check(coefficients[1].a != coefficients[5].a &&
              coefficients[5].a != coefficients[9].a &&
              coefficients[9].a != coefficients[13].a,
          "all four linkage ranges are independently observable");

    PcoFragmentExecutionContext context;
    Check(context.coefficients.size() >= 68,
          "USC context holds four-varying coefficient bank");
    ...
    Check(execution.written_mask == 0x0f,
          "four FITRP values and left-associated FADD write RGBA");
    Check(execution.pixel_outputs[0] == UINT32_C(0x3e3a2e8c) &&
          ...
          "v1+v2+v3+v4 is exact perspective 2/11,1/11,8/11,1");
    ...
    std::cout << "varyings_shader_four_perspective_test: PASS\n";
    return 0;
  } catch (const std::exception &error) {
    std::cerr << "varyings_shader_four_perspective_test: FAIL: "
              << error.what() << '\n';
    return 1;
  }
}
