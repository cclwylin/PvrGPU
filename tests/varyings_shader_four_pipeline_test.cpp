/*
 * GLBench varyings_shader_4 complete event-driven SystemC pipeline test.
 *
 * Four smooth vec4 values travel from the exact public PCO program's
 * VTXOUT4..19 through ClipCull, the Parameter Buffer's shared 1/W plus
 * sixteen varying/W planes, PDS coefficient banks, fragment USC
 * FITRP/WDF/FADD execution, PBE, enabled SLC and fixed-latency DRAM readback.
 * Every FIFO carries only a generation-checked MemoryPool handle. The test
 * derives traffic from linkage/task payloads and independently reconstructs
 * the expected DRAM RGBA bytes; it never injects pixels or counters.
 */
...
void Check(bool condition, const std::string &message) {
  if (!condition)
    throw std::runtime_error(
        "varyings_shader_four pipeline test failed: " + message);
}
...
void CheckVertexPath(const MemoryPool &pool, const PipelineState &state) {
  Check(state.vertex_program_summary.vertex_input_mask == UINT32_C(0x3) &&
        ...
        "vertex PCO consumes VTXIN0..1 and emits VTXOUT0..19");

  const std::vector<ShaderVaryingBinding> bindings =
      LoadArray<ShaderVaryingBinding>(pool, state.shader_varying_bindings);
  Check(bindings.size() == 4 &&
            bindings.size() == VaryingVectorCount(state.functional_case),
        "four VS-to-FS bindings");
  ...
  Check(vertices.size() == 2 * kVertexCount && lanes.size() == kVertexCount &&
            refs.size() == kIndexCount,
        "4x4 lattice and post-transform-cache lane counts");
  ...
  for (std::size_t lane_index = 0; lane_index < lanes.size(); ++lane_index) {
    ...
    for (std::size_t component = 0; component < 4; ++component) {
      ...
      const std::uint32_t quarter = FloatBits(clip[component] * 0.25F);
      for (std::size_t varying = 0; varying < 4; ++varying) {
        Check(lane.vertex_output[4 + varying * 4 + component] == quarter,
              "v1..v4 contain independent c/4 outputs");
      }
    }
    for (std::size_t output = 20;
         output < kPcoVertexOutputRegisterCount; ++output)
      Check(lane.vertex_output[output] == 0,
            "VS writes no output above VTXOUT19");
  }
}
...
void CheckParameterAndPdsPath(const MemoryPool &pool,
  ...
  Check(sets_per_triangle == 17 && dwords_per_task == 68,
        "four vec4 linkages derive seventeen sets and 68 dwords");
  ...
  Check(triangles.size() == kTriangleCount &&
            raster_outputs.size() == kTriangleCount * 3 * 20,
        "ClipCull preserves twenty VTXOUT dwords per triangle vertex");
  for (std::size_t index = 0; index < triangles.size(); ++index) {
    ...
    Check(triangle.key.submit_ordinal == index &&
              ...
              triangle.vertex_output_stride_dwords == 20 &&
              triangle.first_vertex_output_dword == index * 3 * 20,
  ...
  Check(parameters.size() == kTriangleCount &&
        ...
        "one seventeen-set Parameter Buffer range per triangle");
  for (std::size_t index = 0; index < parameters.size(); ++index) {
    ...
    for (std::size_t component = 0; component < 4; ++component) {
      ...
      for (std::size_t varying = 1; varying < 4; ++varying) {
        const ParameterCoefficientSet &other =
            coefficients[first + 1 + varying * 4 + component];
        Check(v1.a == other.a && v1.b == other.b && v1.c == other.c &&
                  v1.pad == 0 && other.pad == 0,
              "independent varying planes contain equal c/4 data");
      }
    }
  }
  ...
  Check(copied.size() == tasks.size() * dwords_per_task,
        "PDS copies 68 coefficient dwords per task");
...
void CheckShaderAndDramPath(const MemoryPool &pool,
  ...
  Check(state.counters.vs_alu_instructions == vs.executed_alu_instructions &&
            state.counters.vs_tex_instructions ==
                vs.executed_tex_instructions &&
            state.counters.vs_memory_instructions ==
                vs.executed_memory_instructions &&
            state.counters.fs_alu_instructions == fs.executed_alu_instructions &&
  ...
  for (const FragmentOutput &output : outputs) {
    ...
    Check(output.pixel_output[0] == FloatBits(expected_x) &&
          ...
          "four interpolated c/4 vectors sum to exact c");
  }
...
int sc_main(int, char **) {
  try {
    ...
    options.test_case = "varyings_shader_4";
    ...
    Check(result.stage == PipelineStage::kFramebufferReady &&
              result.functional_case == FunctionalCase::kVaryingsShaderFour &&
              result.width == kWidth && result.height == kHeight &&
              result.cache_bypass == 0,
          "Gate 14 reaches framebuffer-ready with cache enabled");
    ...
    std::cout << "varyings_shader_four_pipeline_test: PASS\n";
    return 0;
  } catch (const std::exception &error) {
    std::cerr << "varyings_shader_four_pipeline_test: FAIL: " << error.what()
              << '\n';
    return 1;
  }
}
