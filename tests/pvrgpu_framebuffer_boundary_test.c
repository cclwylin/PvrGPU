...
static void
test_mrt_boundary(unsigned samples, bool recorded_draws)
{
   struct pvrgpu_resource colors[3];
   for (unsigned i = 0; i < ARRAY_SIZE(colors); ++i)
      colors[i] = resource(PIPE_FORMAT_R8G8B8A8_UNORM, samples);
   ...
   ctx.color_readback_pending_mask = 7u | PVRGPU_DEPTH_READBACK_PENDING;
   ...
   CHECK(color_calls == 3 && depth_calls == 1);
...
int main(void)
{
   for (unsigned samples = 1; samples <= 16; samples *= 2) {
      test_mrt_boundary(samples, false);
      test_mrt_boundary(samples, true);
   }
   for (unsigned samples = 1; samples <= 16; samples *= 2)
