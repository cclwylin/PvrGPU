/*
 * GLBench varyings_shader_8 complete event-driven SystemC pipeline test.
 *
 * Eight smooth vec4 values travel from exact public PCO VTXOUT4..35 through
 * clipping, the Parameter Buffer's shared 1/W and 32 varying/W planes, PDS
 * coefficient banks, fragment USC FITRP/WDF/FADD execution, PBE, enabled SLC
 * and fixed-latency DRAM readback. FIFO links carry only generation-checked
 * MemoryPool handles. Expected pixels, coefficient traffic and DrawList
 * instruction totals are independently derived from payloads/executed PCO.
 */
...
void Check(bool condition, const std::string &message) {
  if (!condition)
    throw std::runtime_error(
        "varyings_shader_eight pipeline test failed: " + message);
}
...
void CheckVertexPath(const MemoryPool &pool, const PipelineState &state) {
  ...
  for (std::size_t lane_index = 0; lane_index < lanes.size(); ++lane_index) {
    ...
    for (std::size_t component = 0; component < 4; ++component) {
      ...
      const std::uint32_t eighth = FloatBits(clip[component] * 0.125F);
      for (std::size_t varying = 0; varying < 8; ++varying) {
        Check(lane.vertex_output[4 + varying * 4 + component] == eighth,
              "v1..v8 contain independent c/8 outputs");
      }
    }
    for (std::size_t output = 36;
         output < kPcoVertexOutputRegisterCount; ++output)
      Check(lane.vertex_output[output] == 0,
            "VS writes no output above VTXOUT35");
  }
}
...
void CheckParameterAndPdsPath(const MemoryPool &pool,
  ...
  Check(sets_per_triangle == 33 && dwords_per_task == 132,
        "eight vec4 linkages derive 33 sets and 132 dwords");
  ...
  Check(triangles.size() == kTriangleCount &&
            raster_outputs.size() == kTriangleCount * 3 * 36,
        "ClipCull preserves 36 VTXOUT dwords per triangle vertex");
  for (std::size_t index = 0; index < triangles.size(); ++index) {
    ...
    Check(triangle.key.submit_ordinal == index &&
              ...
              triangle.first_vertex_output_dword == index * 3 * 36,
  ...
  Check(parameters.size() == kTriangleCount &&
        ...
        "one 33-set Parameter Buffer range per triangle");
    ...
    for (std::size_t component = 0; component < 4; ++component) {
      ...
      for (std::size_t varying = 1; varying < 8; ++varying) {
        ...
        Check(v1.a == other.a && v1.b == other.b && v1.c == other.c &&
              ...
              "independent varying planes contain equal c/8 data");
      }
    }
  }
  ...
  Check(copied.size() == tasks.size() * dwords_per_task,
        "PDS copies 132 coefficient dwords per task");
...
void CheckShaderAndDramPath(const MemoryPool &pool,
  ...
  for (const FragmentOutput &output : outputs) {
    ...
    Check(output.pixel_output[0] == FloatBits(expected_x) &&
          ...
          "eight interpolated c/8 vectors sum to exact c");
  }
...
int sc_main(int, char **) {
  try {
    ...
    options.test_case = "varyings_shader_8";
    ...
    Check(result.stage == PipelineStage::kFramebufferReady &&
              result.functional_case == FunctionalCase::kVaryingsShaderEight &&
              result.width == kWidth && result.height == kHeight &&
              result.cache_bypass == 0,
          "Gate 15 reaches framebuffer-ready with cache enabled");
    ...
    std::cout << "varyings_shader_eight_pipeline_test: PASS\n";
    return 0;
  } catch (const std::exception &error) {
    std::cerr << "varyings_shader_eight_pipeline_test: FAIL: " << error.what()
              << '\n';
    return 1;
  }
}
