...
/* Real front/back-face pipeline proof: both vertex windings, glFrontFace,
 * vertex reflection, and each cull mode. Oversized triangles require actual
 * clipping. A second shader branches to a real texture instruction on back
 * faces, exercising fragment continuation without injecting model inputs. */
#include <EGL/egl.h>
...
static GLuint shader(GLenum stage, const char *source)
{
   ...
   check(ok,"compile front-facing shader"); return s;
}
static GLuint program(unsigned textured)
{
   ...
   int n = snprintf(fs,sizeof(fs),
      ...
      "void main(){if(gl_FrontFacing){color=uvec4(11,22,33,44);}else{color=%s;}}\n",
   ...
   check(ok,"link front-facing shader"); return p;
}
int main(void)
{
   ...
   for (unsigned textured = 0; textured < 2; ++textured) {
      ...
      for (unsigned reflection = 0; reflection < 2; ++reflection) for (unsigned winding = 0; winding < 2; ++winding)
         for (unsigned front_cw = 0; front_cw < 2; ++front_cw) for (unsigned cull = 0; cull < 3; ++cull) {
            ...
            unsigned clockwise = winding ^ reflection;
            unsigned front = clockwise == front_cw;
            ...
            glFinish(); clean("clipped facing draw");
            ...
            for (unsigned i = 0; i < SIDE * SIDE * 4; ++i) {
               unsigned expected = culled ? 0 : (front ? 11 : 55) + 11 * (i % 4);
               if (pixels[i] != expected) fprintf(stderr,"DRAW%u word%u actual%u expected%u\n",draws,i,pixels[i],expected);
               check(pixels[i] == expected,"all pixels match independent winding/culling oracle");
            }
