#!/usr/bin/env bash
# Direct Mesa frontend -> PvrGPU Gallium driver Phase 5 FBO smoke test.
#
# This exercises GLES2 texture-backed FBO attachment, framebuffer-state changes,
# FBO clear/readback, one triangle draw into the FBO, and flush/finish traffic.
# It is still mostly counter-only: clear readback is verified, but FBO draw
# pixels and real synchronization are not claimed yet.
set -euo pipefail
...
usage() {
    cat <<'EOF'
Usage: ./scripts/run-pvrgpu-mesa-driver-phase5-fbo-smoke.sh [options]
...
Options:
  ...
  --size WxH         Pbuffer size for the default framebuffer. Default: 16x16
  --fbo-size WxH     Texture-backed FBO size. Default: 8x8
  --keep-temp        Keep the temporary C source and binary.

Expected result:
  stdout contains FBO_STATUS=complete, FBO_CLEAR_PIXEL=64,128,191,255,
  FBO_COPY_PIXEL=64,128,191,255, and PHASE5_FBO_DRAW=done.
  driver-counter.txt contains framebuffer-state, clear, copy/blit, draw, and
  flush events.
EOF
}
...
while (($# > 0)); do
    case "$1" in
        ...
        --size)
            ...
            ;;
        --fbo-size)
            (($# >= 2)) || { echo "--fbo-size requires WIDTHxHEIGHT" >&2; exit 2; }
            fbo_size="$2"
            shift 2
            ;;
        --keep-temp)
...
if [[ ! "${fbo_size}" =~ ^([1-9][0-9]*)x([1-9][0-9]*)$ ]]; then
    echo "--fbo-size must be WIDTHxHEIGHT with positive integers" >&2
    exit 2
fi
fbo_width="${BASH_REMATCH[1]}"
fbo_height="${BASH_REMATCH[2]}"
...
if [[ "${output_root}" == "${PVRGPU_WORK_ROOT%/}/out/pvrgpu-mesa-driver-smoke" ]]; then
    output_dir="${output_root}/phase5-fbo-smoke-${timestamp}-$$"
else
...
smoke_tmp="$(mktemp -d "${PVRGPU_TMP_ROOT%/}/pvrgpu-mesa-driver-phase5-fbo-smoke.XXXXXX")"
cleanup() {
    if [[ "${keep_temp}" == false && -n "${smoke_tmp:-}" &&
          "${smoke_tmp}" == "${PVRGPU_TMP_ROOT%/}"/pvrgpu-mesa-driver-phase5-fbo-smoke.* ]]; then
        rm -rf -- "${smoke_tmp}"
    fi
}
trap cleanup EXIT

smoke_source="${smoke_tmp}/pvrgpu_mesa_driver_phase5_fbo_smoke.c"
smoke_binary="${smoke_tmp}/pvrgpu_mesa_driver_phase5_fbo_smoke"
...
int main(int argc, char **argv) {
   ...
   const int fbo_width = argc > 3 ? atoi(argv[3]) : 8;
   const int fbo_height = argc > 4 ? atoi(argv[4]) : 8;
   if (width <= 0 || height <= 0 || fbo_width <= 0 || fbo_height <= 0)
      return 2;
   ...
   if (check_gl("fbo texture setup") != 0)
      return 1;
   ...
   printf("FBO_STATUS=complete\n");

   glViewport(0, 0, fbo_width, fbo_height);
   glPixelStorei(GL_PACK_ALIGNMENT, 1);
   glClearColor(0.25f, 0.5f, 0.75f, 1.0f);
   glClear(GL_COLOR_BUFFER_BIT);
   if (check_gl("fbo clear") != 0)
      return 1;

   GLubyte pixel[4] = {0, 0, 0, 0};
   glReadPixels(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, pixel);
   if (check_gl("fbo readback") != 0)
      return 1;
   printf("FBO_CLEAR_PIXEL=%u,%u,%u,%u\n",
          pixel[0], pixel[1], pixel[2], pixel[3]);
   if (pixel[0] != 64 || pixel[1] != 128 || pixel[2] != 191 ||
       pixel[3] != 255) {
      fprintf(stderr, "unexpected FBO clear pixel\n");
      return 1;
   }
   ...
   glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, fbo_width, fbo_height, 0,
                GL_RGBA, GL_UNSIGNED_BYTE, NULL);
   if (check_gl("copy texture setup") != 0)
      return 1;

   glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0,
                       fbo_width, fbo_height);
   if (check_gl("fbo copy texture") != 0)
      return 1;
   ...
   if (status != GL_FRAMEBUFFER_COMPLETE) {
      fprintf(stderr, "copy FBO incomplete: status=0x%04x\n", status);
      return 1;
   }
   pixel[0] = pixel[1] = pixel[2] = pixel[3] = 0;
   glReadPixels(0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, pixel);
   if (check_gl("copy texture readback") != 0)
      return 1;
   printf("FBO_COPY_PIXEL=%u,%u,%u,%u\n",
          pixel[0], pixel[1], pixel[2], pixel[3]);
   if (pixel[0] != 64 || pixel[1] != 128 || pixel[2] != 191 ||
       pixel[3] != 255) {
      fprintf(stderr, "unexpected FBO copy pixel\n");
      return 1;
   }
   ...
   glFlush();
   glFinish();
   if (check_gl("fbo draw/flush") != 0)
      return 1;
   printf("PHASE5_FBO_DRAW=done\n");
   ...
   glClear(GL_COLOR_BUFFER_BIT);
   if (check_gl("default framebuffer clear") != 0)
      return 1;
...
env \
    ...
    "${smoke_binary}" "${width}" "${height}" "${fbo_width}" "${fbo_height}" \
    >"${stdout_log}" \
    2>"${stderr_log}"
...
grep -Fx "FBO_STATUS=complete" "${stdout_log}" >/dev/null
grep -Fx "FBO_CLEAR_PIXEL=64,128,191,255" "${stdout_log}" >/dev/null
grep -Fx "FBO_COPY_PIXEL=64,128,191,255" "${stdout_log}" >/dev/null
grep -Fx "PHASE5_FBO_DRAW=done" "${stdout_log}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=set_framebuffer_state width=${fbo_width} height=${fbo_height} nr_cbufs=1 has_cbuf0=1" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=clear_color width=${fbo_width} height=${fbo_height}" "${counter_out}" >/dev/null
if ! grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=resource_copy_region" "${counter_out}" >/dev/null &&
   ! grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=blit" "${counter_out}" >/dev/null; then
    echo "PvrGPU driver did not emit resource_copy_region or blit in: ${counter_out}" >&2
    exit 1
fi
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=draw_triangles" "${counter_out}" >/dev/null
grep -F "framebuffer=${fbo_width}x${fbo_height}" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=flush" "${counter_out}" >/dev/null

if [[ "${keep_temp}" == true ]]; then
    printf 'temp_dir=%s\n' "${smoke_tmp}" >"${output_dir}/temp.txt"
fi

echo "PvrGPU Mesa driver Phase 5 FBO smoke PASS"
