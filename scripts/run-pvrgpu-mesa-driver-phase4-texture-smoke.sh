#!/usr/bin/env bash
# Direct Mesa frontend -> PvrGPU Gallium driver Phase 4 texture smoke test.
#
# This exercises GLES2 texture upload, sampler state, sampler view binding, and
# a textured triangle draw. It is counter-only: no pixels are compared because
# the driver still does not lower texture sampling into a real model command.
set -euo pipefail
...
usage() {
    cat <<'EOF'
Usage: ./scripts/run-pvrgpu-mesa-driver-phase4-texture-smoke.sh [options]
...
Expected result:
  driver-counter.txt contains texture upload/bind events and
  event=draw_textured_triangles.
EOF
}
...
if [[ "${output_root}" == "${PVRGPU_WORK_ROOT%/}/out/pvrgpu-mesa-driver-smoke" ]]; then
    output_dir="${output_root}/phase4-texture-smoke-${timestamp}-$$"
else
...
smoke_tmp="$(mktemp -d "${PVRGPU_TMP_ROOT%/}/pvrgpu-mesa-driver-phase4-texture-smoke.XXXXXX")"
cleanup() {
    if [[ "${keep_temp}" == false && -n "${smoke_tmp:-}" &&
          "${smoke_tmp}" == "${PVRGPU_TMP_ROOT%/}"/pvrgpu-mesa-driver-phase4-texture-smoke.* ]]; then
        rm -rf -- "${smoke_tmp}"
    fi
}
trap cleanup EXIT

smoke_source="${smoke_tmp}/pvrgpu_mesa_driver_phase4_texture_smoke.c"
smoke_binary="${smoke_tmp}/pvrgpu_mesa_driver_phase4_texture_smoke"
...
int main(int argc, char **argv) {
   ...
   if (texture_uniform < 0) {
      fprintf(stderr, "uniform u_texture was not found\n");
      return 1;
   }
   ...
   glFinish();
   if (check_gl("glDrawArrays textured") != 0)
      return 1;

   printf("PHASE4_TEXTURE_DRAW=done\n");
...
grep -Fx "PHASE4_TEXTURE_DRAW=done" "${stdout_log}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=texture_subdata" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=create_sampler_state" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=bind_sampler_states" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=create_sampler_view" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=set_sampler_views" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=draw_textured_triangles" "${counter_out}" >/dev/null

if [[ "${keep_temp}" == true ]]; then
    printf 'temp_dir=%s\n' "${smoke_tmp}" >"${output_dir}/temp.txt"
fi

echo "PvrGPU Mesa driver Phase 4 texture smoke PASS"
