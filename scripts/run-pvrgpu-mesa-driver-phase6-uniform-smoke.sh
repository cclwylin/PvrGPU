#!/usr/bin/env bash
# Direct Mesa frontend -> PvrGPU Gallium driver Phase 6 uniform smoke test.
#
# This exercises GLES2 uniform upload into Gallium constant-buffer state and one
# draw that observes those constants at the PvrGPU driver boundary. It is
# counter-only: uniform-driven pixels are not claimed correct yet.
set -euo pipefail
...
usage() {
    cat <<'EOF'
Usage: ./scripts/run-pvrgpu-mesa-driver-phase6-uniform-smoke.sh [options]
...
Expected result:
  stdout contains PHASE6_UNIFORM_DRAW=done and driver-counter.txt contains
  set_constant_buffer with real fragment uniform data plus
  event=draw_uniform_triangles.
EOF
}
...
if [[ "${output_root}" == "${PVRGPU_WORK_ROOT%/}/out/pvrgpu-mesa-driver-smoke" ]]; then
    output_dir="${output_root}/phase6-uniform-smoke-${timestamp}-$$"
else
...
smoke_tmp="$(mktemp -d "${PVRGPU_TMP_ROOT%/}/pvrgpu-mesa-driver-phase6-uniform-smoke.XXXXXX")"
cleanup() {
    if [[ "${keep_temp}" == false && -n "${smoke_tmp:-}" &&
          "${smoke_tmp}" == "${PVRGPU_TMP_ROOT%/}"/pvrgpu-mesa-driver-phase6-uniform-smoke.* ]]; then
        rm -rf -- "${smoke_tmp}"
    fi
}
trap cleanup EXIT

smoke_source="${smoke_tmp}/pvrgpu_mesa_driver_phase6_uniform_smoke.c"
smoke_binary="${smoke_tmp}/pvrgpu_mesa_driver_phase6_uniform_smoke"
...
int main(int argc, char **argv) {
   ...
   const char *fs_source =
      ...
      "void main() {\n"
      "   gl_FragColor = (u_color[0] + u_color[1] + u_color[2] + u_color[3] +\n"
      "                   u_color[4] + u_color[5] + u_color[6] + u_color[7]) * 0.125;\n"
      "}\n";
   ...
   GLint color_uniform = glGetUniformLocation(program, "u_color[0]");
   if (color_uniform < 0)
      color_uniform = glGetUniformLocation(program, "u_color");
   if (color_uniform < 0) {
      fprintf(stderr, "uniform u_color was not found\n");
      return 1;
   }
   glUniform4fv(color_uniform, 8, colors);
   if (check_gl("glUniform4fv") != 0)
      return 1;
   ...
   glFinish();
   if (check_gl("glDrawArrays uniform") != 0)
      return 1;

   printf("PHASE6_UNIFORM_DRAW=done\n");
...
grep -Fx "PHASE6_UNIFORM_DRAW=done" "${stdout_log}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=set_constant_buffer stage=fragment index=0 has_buffer=1" "${counter_out}" >/dev/null
grep -F "has_words=1" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=draw_uniform_triangles" "${counter_out}" >/dev/null

if [[ "${keep_temp}" == true ]]; then
    printf 'temp_dir=%s\n' "${smoke_tmp}" >"${output_dir}/temp.txt"
fi

echo "PvrGPU Mesa driver Phase 6 uniform smoke PASS"
