#!/usr/bin/env bash
# Direct Mesa frontend -> PvrGPU Gallium driver triangle smoke test.
#
# This intentionally avoids RenderDoc, RDC translation, and the SystemC model.
# It proves that a GLES2 shader + client vertex array + glDrawArrays triangle
# reaches src/gallium/drivers/pvrgpu and produces deterministic driver counters.
set -euo pipefail
...
usage() {
    cat <<'EOF'
Usage: ./scripts/run-pvrgpu-mesa-driver-triangle-smoke.sh [options]
...
Options:
  ...
  --size WxH         Pbuffer size for the triangle draw. Default: 16x16
  --keep-temp        Keep the temporary C source and binary.

Expected result:
  The smoke creates driver-counter.txt containing event=draw_triangles.
EOF
}
...
if [[ "${output_root}" == "${PVRGPU_WORK_ROOT%/}/out/pvrgpu-mesa-driver-smoke" ]]; then
    output_dir="${output_root}/triangle-smoke-${timestamp}-$$"
else
...
smoke_tmp="$(mktemp -d "${PVRGPU_TMP_ROOT%/}/pvrgpu-mesa-driver-triangle-smoke.XXXXXX")"
cleanup() {
    if [[ "${keep_temp}" == false && -n "${smoke_tmp:-}" &&
          "${smoke_tmp}" == "${PVRGPU_TMP_ROOT%/}"/pvrgpu-mesa-driver-triangle-smoke.* ]]; then
        rm -rf -- "${smoke_tmp}"
    fi
}
trap cleanup EXIT

smoke_source="${smoke_tmp}/pvrgpu_mesa_driver_triangle_smoke.c"
smoke_binary="${smoke_tmp}/pvrgpu_mesa_driver_triangle_smoke"
...
int main(int argc, char **argv) {
   ...
   glFinish();
   if (check_gl("glDrawArrays") != 0)
      return 1;

   printf("TRIANGLE_DRAW=done\n");
...
grep -Fx "TRIANGLE_DRAW=done" "${stdout_log}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=create_shader stage=vertex" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=create_shader stage=fragment" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=bind_shader stage=vertex bound=1" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=bind_shader stage=fragment bound=1" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=create_vertex_elements" "${counter_out}" >/dev/null
grep -F "schema=pvrgpu.driver-counter.v1 producer=pvrgpu-gallium-driver event=set_vertex_buffers" "${counter_out}" >/dev/null
...
grep -Fx "command=draw_triangle" "${command_out}" >/dev/null
grep -Fx "vertex0_bits=3212836864,3212836864" "${command_out}" >/dev/null
grep -Fx "fragment_color_bits=1065353216,0,0,1065353216" "${command_out}" >/dev/null

if [[ "${keep_temp}" == true ]]; then
    printf 'temp_dir=%s\n' "${smoke_tmp}" >"${output_dir}/temp.txt"
fi

echo "PvrGPU Mesa driver triangle smoke PASS"
