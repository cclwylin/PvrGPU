...
int main(void)
{
   ...
   for (unsigned kind = 0; kind < 5; ++kind) {
      ...
      require(binary.control.shader.abi.uniform_buffer_descriptor_start == 8, "TCS descriptor prefix");
      require(binary.evaluation.shader.abi.uniform_buffer_descriptor_start == 4, "TES descriptor prefix");
   ...
   for (unsigned invalid = 0; invalid < 7; ++invalid) {
      ...
      if (invalid == 4) tes->info.num_textures = 1;
