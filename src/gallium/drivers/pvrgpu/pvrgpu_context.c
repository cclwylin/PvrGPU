...
#include "pvrgpu_push_constants.h"
...
{
   return !ctx || !ctx->framebuffer.nr_cbufs ? PVRGPU_DRIVER_COMMAND_FORMAT_RGBA8 :
      pvrgpu_command_format_for_surface(ctx->framebuffer.cbufs[0].format);
}

static bool
pvrgpu_framebuffer_has_mixed_color_formats(const struct pvrgpu_context *ctx)
{
   for (unsigned target = 1; target < ctx->framebuffer.nr_cbufs; ++target)
      if (ctx->framebuffer.cbufs[target].format != ctx->framebuffer.cbufs[0].format)
         return true;
   return false;
}

static bool
pvrgpu_framebuffer_color_transport_is_bounded(const struct pvrgpu_context *ctx)
{
                                      ...
                                      uint32_t *destination)
{
   ...
   case MESA_SHADER_TESS_CTRL:
      *destination = PVRGPU_SYSTEMC_PCO_SHADER_STAGE_TESS_CONTROL;
      return true;
   case MESA_SHADER_TESS_EVAL:
      *destination = PVRGPU_SYSTEMC_PCO_SHADER_STAGE_TESS_EVALUATION;
      return true;
   ...
   uint32_t *out_vs_invocations)
{
   ...
   for (unsigned index = 0; index < draw_count; ++index) {
      ...
      primitives += (draw->tessellation && draw->tessellation->input_vertices ?
         assembled / draw->tessellation->input_vertices :
         pvrgpu_array_primitive_count(draw->primitive_mode, assembled)) * instances;
   }
...
bool
pvrgpu_expand_instanced_connected_indices(
...
{
   if (!out_indices || !out_index_count || !out_mode || !instances ||
       !vertices_per_instance || vertices_per_instance > UINT32_MAX / instances ||
       (source_indices ? (source_index_size != 1 && source_index_size != 2 && source_index_size != 4)
                       : (source_index_size != 0 || element_count > vertices_per_instance)))
      return false;
   const bool lines = mode == MESA_PRIM_LINE_STRIP || mode == MESA_PRIM_LINE_LOOP;
   if (!lines && mode != MESA_PRIM_TRIANGLE_STRIP && mode != MESA_PRIM_TRIANGLE_FAN)
      return false;
   const unsigned width = lines ? 2u : 3u;
   const unsigned primitives = pvrgpu_array_primitive_count(mode, element_count);
   const uint64_t per_instance = (uint64_t)primitives * width;
   if (per_instance > UINT32_MAX / instances)
      return false;
   const uint64_t count = per_instance * instances;
   if (!count || count > UINT32_MAX || count > SIZE_MAX / sizeof(uint32_t))
      return false;
   ...
   for (unsigned instance = 0; instance < instances; ++instance) {
      for (unsigned primitive = 0; primitive < primitives; ++primitive) {
         unsigned elements[3] = {primitive, primitive + 1, primitive + 2};
         if (mode == MESA_PRIM_LINE_LOOP)
            elements[1] %= element_count;
         else if (mode == MESA_PRIM_TRIANGLE_FAN)
            elements[0] = 0;
         else if (mode == MESA_PRIM_TRIANGLE_STRIP && (primitive & 1)) {
            elements[0] = primitive + 1;
            elements[1] = primitive;
         }
         for (unsigned component = 0; component < width; ++component) {
            uint32_t vertex = elements[component];
            if (source_indices) {
               const uint8_t *source = (const uint8_t *)source_indices +
                  (size_t)vertex * source_index_size;
               if (source_index_size == 1) vertex = *source;
               else if (source_index_size == 2) {
                  uint16_t value;
                  memcpy(&value, source, sizeof(value));
                  vertex = value;
               } else memcpy(&vertex, source, sizeof(vertex));
            }
            if (vertex >= vertices_per_instance) {
               free(indices);
               return false;
            }
            indices[destination++] = instance * vertices_per_instance + vertex;
         }
      }
   }
   *out_indices = indices;
   *out_index_count = (unsigned)count;
   *out_mode = lines ? MESA_PRIM_LINES : MESA_PRIM_TRIANGLES;
   return true;
}
...
pvrgpu_shadow_gather_sampler_supported(const struct pvrgpu_context *ctx,
                                      ...
                                      const struct pipe_sampler_state *sampler)
{
   /* Gather returns four unfiltered depths; native shader ALU compares each
    * tap. Linear image filtering therefore needs no PCF implementation here.
    * Prove that this slot has no ordinary shadow samples before relaxing the
    * nearest-only rule. The exposed image is rebased by the existing snapshot
    * code, so a single exposed mip also has model base level zero. */
   ...
   const char **reason)
{
   ...
   if (!reason)
      reason = &ignored;
   uint32_t api_stage;
   if (!ctx || !destination || !out_bytes || slot >= PVRGPU_PCO_MAX_TEXTURES ||
       !pvrgpu_systemc_shader_stage_from_mesa(stage, &api_stage)) {
      *reason = "arguments";
      return false;
   }
   ...
   const bool cube_array_view = view->target == PIPE_TEXTURE_CUBE_ARRAY;
   const bool layered_view = array_view || cube_array_view;
   /*
   ...
   if (sample_count > 8U || (sample_count & (sample_count - 1U)) != 0 ||
       (multisample_view && (volume_view || cube_view || cube_array_view ||
                            view->texture->last_level != 0))) {
      *reason = "sample_layout";
      return false;
   }
   const unsigned resource_layers = layered_view ? view->texture->array_size
   ...
   if (layers == 0U || layers > 4096U || (layered_view && layers > 2048U)) {
      *reason = "layers";
      return false;
   }

   const enum pipe_format format = pvrgpu_effective_sampled_format(view);
   const bool cube_array_depth_view = cube_array_view &&
      util_format_has_depth(util_format_description(format));
   /* Cube-array addressing uses complete six-face cubes, rebased at the
    * first exposed cube and mip. Depth views use the existing exact unpack
    * and view-swizzle path below, producing RGBA32F storage; the independent
    * compare metadata selects shadow behavior in every native shader stage. */
   if (cube_array_view &&
       (view->texture->target != PIPE_TEXTURE_CUBE_ARRAY ||
        view->texture->width0 != view->texture->height0 ||
        view->texture->nr_samples != 0 || view->texture->nr_storage_samples > 1 ||
        resource_layers % 6U != 0 || first_layer % 6U != 0 || layers % 6U != 0 ||
        (util_format_is_depth_or_stencil(format) && !cube_array_depth_view) ||
   ...
   const enum pipe_format storage_format = integer_view ?
      (util_format_is_pure_uint(format) ? PIPE_FORMAT_R32G32B32A32_UINT :
                                         PIPE_FORMAT_R32G32B32A32_SINT) :
      canonical_float_view ? PIPE_FORMAT_R32G32B32A32_FLOAT : format;
   ...
   const struct pipe_sampler_state *state = &sampler->state;
   const bool shadow_gather = stage == MESA_SHADER_FRAGMENT && ctx->fs &&
      pvrgpu_pco_fragment_has_shadow_gather(ctx->fs->nir, slot);
   /* A shadow gather must never fall through the legacy nearest/query
    * exceptions, even for a disabled comparison or multisample binding. In
    * particular, canonicalizing a constant red swizzle before shader compare
    * is not the API's gather-constant swizzle after comparison. */
   if ((shadow_gather &&
        !pvrgpu_shadow_gather_sampler_supported(ctx, stage, slot, view, state)) ||
       (!shadow_gather && !multisample_view &&
        state->compare_mode != PIPE_TEX_COMPARE_NONE &&
   ...
   uint8_t *bytes = malloc((size_t)storage_size);
   if (!bytes) {
      *reason = "allocation";
      return false;
   }
   if (!canonical_view) {
       ...
       * apply the actual sampler-view swizzle once before the identity
       * descriptor is consumed by the model's integer texture datapath. */
      const unsigned swizzle[4] = { view->swizzle_r, view->swizzle_g,
                                    view->swizzle_b, view->swizzle_a };
         ...
         for (unsigned layer = 0; layer < slices; ++layer) {
            for (unsigned y = 0; y < level_height; ++y) {
               ...
               for (unsigned x = 0; x < level_width * sample_count; ++x) {
                  ...
                  memcpy(bytes + output_offset, result, sizeof(result));
                  output_offset += sizeof(result);
               }
            }
         }
      }
   ...
   destination->format = util_format_name(storage_format);
   destination->bytes = bytes;
   destination->bytes_size = (size_t)storage_size;
   destination->declared_bytes_size = (size_t)storage_size;
   destination->mip_count = mip_count;
   uintptr_t mip_offset = 0;
   for (unsigned level = 0; level < mip_count; ++level) {
   ...
   destination->texture_kind =
      cube_array_view ? 4U : cube_view ? 3U : volume_view ? 2U : array_view ? 1U : 0U;
...
struct pvrgpu_array_primitive_draw {
   ...
   uint8_t *initial_depth_attachment_bytes;
   /*
   ...
   uint32_t geometry_shared_words[PVRGPU_COLOR_PRIMITIVE_UNIFORM_DWORDS];
   uint32_t control_shared_words[PVRGPU_COLOR_PRIMITIVE_UNIFORM_DWORDS];
   uint32_t evaluation_shared_words[PVRGPU_COLOR_PRIMITIVE_UNIFORM_DWORDS];
   struct pvrgpu_systemc_pco_uniform_buffer
      uniform_buffers[5 * PVRGPU_SYSTEMC_MAX_UNIFORM_BUFFERS_PER_STAGE];
    ...
    */
   struct pvrgpu_systemc_pco_sequence_texture
      textures[5 * PVRGPU_PCO_MAX_TEXTURES];
   uint8_t *texture_bytes[5 * PVRGPU_PCO_MAX_TEXTURES];
   unsigned texture_count;
   struct pvrgpu_pco_graphics_binary binary;
   struct pvrgpu_pco_geometry_binary geometry;
   struct pvrgpu_pco_tessellation_pipeline_binary tessellation_binary;
   struct pvrgpu_systemc_tessellation tessellation;
   struct pvrgpu_systemc_stream_output stream_output;
   struct pvrgpu_systemc_stream_output_binding
      stream_output_bindings[PVRGPU_SYSTEMC_MAX_STREAM_OUTPUT_BINDINGS];
   struct pvrgpu_systemc_stream_output_target
      stream_output_payloads[PIPE_MAX_SO_BUFFERS];
   ...
   struct pvrgpu_systemc_graphics_shader_buffers graphics_buffers;
   struct pvrgpu_systemc_shader_buffer_resource
      graphics_buffer_resources[PVRGPU_SYSTEMC_MAX_SHADER_BUFFER_RESOURCES];
   struct pvrgpu_systemc_shader_buffer_binding
      graphics_buffer_bindings[PVRGPU_SYSTEMC_MAX_SHADER_BUFFER_BINDINGS];
};
...
pvrgpu_array_primitive_draw_destroy(struct pvrgpu_array_primitive_draw **slot)
{
   ...
   pvrgpu_pco_geometry_binary_finish(&draw->geometry);
   pvrgpu_pco_tessellation_pipeline_binary_finish(&draw->tessellation_binary);
   ...
   pvrgpu_compute_snapshot_finish(&draw->graphics_buffer_snapshot);
   ...
   for (unsigned buffer = 0; buffer < PIPE_MAX_SO_BUFFERS; ++buffer) {
      free(draw->stream_output_bytes[buffer]);
      pipe_so_target_reference(&draw->stream_output_targets[buffer], NULL);
   }
   FREE(draw);
   *slot = NULL;
}
...
static bool
pvrgpu_capture_graphics_buffers(
   const struct pvrgpu_context *ctx,
   const struct pvrgpu_pco_graphics_binary *graphics,
   const struct pvrgpu_pco_geometry_binary *geometry,
   const struct pvrgpu_pco_tessellation_pipeline_binary *tessellation,
   uint32_t *vertex_shared,
   uint32_t *fragment_shared,
   uint32_t *geometry_shared,
   uint32_t *control_shared,
   uint32_t *evaluation_shared,
   struct pvrgpu_systemc_graphics_shader_buffers *payload,
...
{
   if (!ctx || !graphics || !vertex_shared || !fragment_shared || !payload ||
       !snapshot || !resources || !bindings ||
       ((ctx->gs != NULL) != (geometry != NULL)) ||
       (!!(ctx->tcs && ctx->tes) != (tessellation != NULL)) ||
       (ctx->gs && !geometry_shared) ||
       (tessellation && (!control_shared || !evaluation_shared)))
      return false;
   const struct {
      mesa_shader_stage stage;
      ...
      {MESA_SHADER_VERTEX, PVRGPU_SYSTEMC_PCO_SHADER_STAGE_VERTEX,
       &graphics->vertex, vertex_shared},
      {MESA_SHADER_FRAGMENT, PVRGPU_SYSTEMC_PCO_SHADER_STAGE_FRAGMENT,
       &graphics->fragment, fragment_shared},
      {MESA_SHADER_GEOMETRY, PVRGPU_SYSTEMC_PCO_SHADER_STAGE_GEOMETRY,
       geometry ? &geometry->shader : NULL, geometry_shared},
      {MESA_SHADER_TESS_CTRL, PVRGPU_SYSTEMC_PCO_SHADER_STAGE_TESS_CONTROL,
       tessellation ? &tessellation->control.shader : NULL, control_shared},
      {MESA_SHADER_TESS_EVAL, PVRGPU_SYSTEMC_PCO_SHADER_STAGE_TESS_EVALUATION,
       tessellation ? &tessellation->evaluation.shader : NULL, evaluation_shared},
   };
   for (unsigned stage_index = 0; stage_index < ARRAY_SIZE(stages); ++stage_index) {
      const __typeof__(stages[0]) *stage = &stages[stage_index];
      if (!stage->binary)
         continue;
      const struct pvrgpu_pco_storage_buffer_abi *compiled =
         &stage->binary->storage;
      payload->storage[stage->public_stage] =
         (struct pvrgpu_systemc_storage_buffer_abi){
         .descriptor_start = compiled->descriptor_start,
         .descriptor_count = compiled->descriptor_count,
         .used_mask = compiled->used_mask,
         .read_mask = compiled->read_mask,
         .write_mask = compiled->write_mask,
      };
      if (compiled->descriptor_count > PIPE_MAX_SHADER_BUFFERS ||
          (uint64_t)compiled->descriptor_start +
             4u * compiled->descriptor_count > stage->binary->abi.shareds)
         return false;
      for (unsigned slot = 0; slot < compiled->descriptor_count; ++slot) {
         const uint32_t bit = UINT32_C(1) << slot;
         if (!(compiled->used_mask & bit))
            continue;
   ...
   payload->resources = snapshot->resource_count ? resources : NULL;
   payload->resource_count = snapshot->resource_count;
   payload->bindings = snapshot->binding_count ? bindings : NULL;
   payload->binding_count = snapshot->binding_count;
   return true;
}
...
static bool
pvrgpu_capture_stream_output(const struct pvrgpu_context *ctx,
                              struct pvrgpu_array_primitive_draw *recorded)
{
   ...
   if (!info->num_outputs || info->num_outputs > PVRGPU_SYSTEMC_MAX_STREAM_OUTPUT_BINDINGS)
      return false;
   unsigned used_buffers = 0;
   for (unsigned output = 0; output < info->num_outputs; ++output) {
      unsigned ordinal = 0, location = 0;
      const unsigned register_index = info->output[output].register_index;
      for (; location < 64; ++location) {
         if (!(producer->nir->info.outputs_written & BITFIELD64_BIT(location)))
            continue;
         if (ordinal++ == register_index)
            break;
      }
      const unsigned first = info->output[output].start_component;
      const unsigned count = info->output[output].num_components;
      const unsigned buffer = info->output[output].output_buffer;
      if (location == 64 || !count || count > 4 || buffer >= PIPE_MAX_SO_BUFFERS ||
          info->output[output].stream ||
          first + count > recorded->binary.vertex_output_count[location] ||
          info->output[output].dst_offset + count > info->stride[buffer])
         return false;
      recorded->stream_output_bindings[output] =
         (struct pvrgpu_systemc_stream_output_binding){
            .output_dword = recorded->binary.vertex_output_start[location] + first,
            .num_components = count,
            .output_buffer = buffer,
            .dst_offset_dwords = info->output[output].dst_offset,
            .stream = info->output[output].stream,
         };
   ...
   recorded->stream_output.bindings = recorded->stream_output_bindings;
   recorded->stream_output.binding_count = info->num_outputs;
   for (unsigned buffer = 0; buffer < ctx->num_stream_output_targets; ++buffer) {
      struct pipe_stream_output_target *base = ctx->stream_output_targets[buffer];
      if (!base || !(used_buffers & (1u << buffer)))
         continue;
      struct pvrgpu_stream_output_target *target = (struct pvrgpu_stream_output_target *)base;
      struct pvrgpu_resource *resource = pvrgpu_resource(base->buffer);
      if (base->context != &ctx->base || !resource->data || !resource->size ||
          resource->size > PVRGPU_SYSTEMC_MAX_STREAM_OUTPUT_RESOURCE_BYTES ||
          base->buffer_offset > resource->size ||
          base->buffer_size > resource->size - base->buffer_offset)
         return false;
      const unsigned slot = recorded->stream_output.target_count++;
      uint8_t *bytes = malloc(resource->size);
      recorded->stream_output_bytes[slot] = bytes;
      if (!bytes)
         return false;
      memcpy(bytes, resource->data, resource->size);
      pipe_so_target_reference(&recorded->stream_output_targets[slot], base);
      recorded->stream_output_payloads[slot] =
         (struct pvrgpu_systemc_stream_output_target){
            .output_buffer = buffer,
            .resource_token = (uintptr_t)base->buffer,
            .target_token = (uintptr_t)base,
            .bytes = bytes,
            .bytes_size = resource->size,
            .buffer_offset = base->buffer_offset,
            .buffer_size = base->buffer_size,
            .internal_offset = target->internal_offset,
            .stride_dwords = info->stride[buffer],
         };
   }
   recorded->stream_output.targets = recorded->stream_output.target_count ?
      recorded->stream_output_payloads : NULL;
   recorded->command.stream_output = &recorded->stream_output;
   return true;
}
   ...
   unsigned target, uint8_t **out_pixels, size_t *out_size)
{
   ...
   const unsigned framebuffer_width = ctx->framebuffer.width;
   ...
   if (!texture || !resource->data || width == 0 || height == 0 ||
       surface->level >= resource->level_count ||
       surface->level >= PIPE_MAX_TEXTURE_LEVELS ||
       framebuffer_width > u_minify(texture->width0, surface->level) ||
   ...
   if (surface->first_layer >= layers || surface->last_layer >= layers || !description ||
       description->block.width != 1 || description->block.height != 1 ||
       description->block.depth != 1 ||
   ...
   if (!pvrgpu_surface_span(resource, surface, row_bytes, height, capture_layers, &offset))
      return false;

   const size_t size = (size_t)width * height * bytes_per_pixel * capture_layers;
   ...
   for (unsigned layer = 0; layer < capture_layers; ++layer) {
...
static bool
pvrgpu_capture_initial_depth_attachment(const struct pvrgpu_context *ctx,
                                        struct pvrgpu_array_primitive_draw *recorded)
{
   ...
   if (!texture)
      return true;
   const struct pvrgpu_resource *resource = (const struct pvrgpu_resource *)texture;
   if (!resource->data || surface->format != texture->format ||
       surface->level >= resource->level_count ||
       surface->last_layer >= pvrgpu_resource_render_layer_count(texture, surface->level))
      return false;
   const unsigned samples = MAX2(1, recorded->command.raster_samples);
   const unsigned bpp = util_format_get_blocksize(surface->format);
   const unsigned width = ctx->framebuffer.width;
   const unsigned height = ctx->framebuffer.height;
   if (!width || !height || width > u_minify(texture->width0, surface->level) ||
       height > u_minify(texture->height0, surface->level))
      return false;
   const size_t row_size = (size_t)width * samples * bpp;
   const unsigned capture_layers = MAX2(1, recorded->command.framebuffer_layers);
   if (row_size > SIZE_MAX / height / capture_layers)
      return false;
   const size_t size = row_size * height * capture_layers;
   size_t offset = 0;
   const unsigned stride = resource->level_strides[surface->level];
   if (!pvrgpu_surface_span(resource, surface, row_size, height, capture_layers, &offset))
      return false;
   uint8_t *pixels = malloc(size);
   if (!pixels)
      return false;
   for (unsigned layer = 0; layer < capture_layers; ++layer)
   for (unsigned y = 0; y < height; ++y)
      memcpy(pixels + ((size_t)layer * height + y) * row_size,
             resource->data + offset + (size_t)layer * resource->level_layer_strides[surface->level] +
                (size_t)y * stride, row_size);
...
pvrgpu_context_end_frame_at_readback(struct pvrgpu_context *ctx)
{
   ...
   if (ctx->array_primitive_draw_count != 0) {
      ...
      /* An RDC sequence may already own a successful submission. Otherwise
       * a rejected submission must invalidate queries spanning these draws,
       * even when no new generation reached the model. */
      if (!emitted && !ctx->array_primitive_sequence_owns_command)
   ...
   pvrgpu_query_collect_completed(ctx);
   ...
   ctx->driver_indexed_quad_command_locked = false;
   ctx->indexed_quad_draws = 0;
...
pvrgpu_emit_array_primitive_sequence_command(struct pvrgpu_context *ctx)
{
   ...
   for (unsigned ordinal = 0; ordinal < ctx->array_primitive_draw_count;
      ...
      for (unsigned target = 0; target < first->render_target_count; ++target) {
         if (strcmp(pvrgpu_color_format_at(recorded->command.format,
                       recorded->command.color_attachment_format_count,
                       recorded->command.color_attachment_formats, target),
                    pvrgpu_color_format_at(first->format,
                       first->color_attachment_format_count,
                       first->color_attachment_formats, target))) {
            pvrgpu_counter_eventf("draw_array_primitive_sequence_error",
               "stage=assemble ordinal=%u reason=attachment_format_mismatch target=%u",
               ordinal, target);
            free(sequence_textures);
            free(draws);
            return false;
   ...
   for (unsigned ordinal = 0; ordinal < ctx->array_primitive_draw_count;
      ...
      if (draw->tessellation) {
         ...
         if (!t->input_vertices || !t->vertices_per_instance || assembled % t->vertices_per_instance ||
             t->vertices_per_instance % t->input_vertices) {
            free(sequence_textures);
            free(draws);
            return false;
         }
         ia_primitives += assembled / t->input_vertices;
      } else if (draw->geometry_pco_size) {
         const uint32_t per_instance = draw->geometry_vertices_per_instance;
         if (!per_instance || assembled % per_instance) {
            free(sequence_textures);
            free(draws);
            return false;
         }
         ia_primitives += pvrgpu_array_primitive_count(draw->primitive_mode, per_instance) *
                          (assembled / per_instance);
      } else {
         ia_primitives += pvrgpu_array_primitive_count(draw->primitive_mode, assembled);
      }
   }
   ...
   for (unsigned ordinal = 0; ordinal < ctx->array_primitive_draw_count; ++ordinal)
      if (draws[ordinal].geometry_pco_size)
         command.clip_invocations = 0; /* native GS emission decides clip work */
   command.render_target_count = first->render_target_count;
   command.color_attachment_format_count = first->color_attachment_format_count;
   memcpy(command.color_attachment_formats, first->color_attachment_formats,
          sizeof(command.color_attachment_formats));
   ...
   pvrgpu_note_driver_draw_command_emitted();
   pvrgpu_note_current_color_readback_pending(ctx);
   /* Stream output completes while the recording still owns its target
    * references. The following draw snapshots this actual modeled result,
    * including the append cursor; application rebinding never invalidates
   ...
   for (unsigned ordinal = 0; ordinal < ctx->array_primitive_draw_count; ++ordinal) {
      ...
      struct pvrgpu_compute_snapshot *buffers =
         &recorded->graphics_buffer_snapshot;
      for (unsigned resource_index = 0;
         ...
         if (!pvrgpu_read_shader_image(&readback, error, sizeof(error))) {
            ++ctx->query_statistics_failures;
            pvrgpu_counter_eventf("graphics_buffer_readback_error",
                                  "reason=%s", error);
            return false;
         }
         pvrgpu_counter_eventf("graphics_buffer_readback",
            "generation=%llu resource=%u bytes=%zu",
            (unsigned long long)readback.submission_generation,
            resource_index, readback.bytes_size);
      }
      ...
      for (unsigned buffer = 0; buffer < recorded->stream_output.target_count; ++buffer) {
         ...
         struct pvrgpu_stream_output_target *target =
            (struct pvrgpu_stream_output_target *)recorded->stream_output_targets[buffer];
         struct pvrgpu_resource *resource = pvrgpu_resource(target->base.buffer);
         ...
         if (!pvrgpu_read_stream_output(&readback, error, sizeof(error)) ||
             readback.internal_offset > target->base.buffer_size) {
            ++ctx->query_statistics_failures;
            pvrgpu_counter_eventf("stream_output_readback_error", "reason=%s", error);
            return false;
         }
         target->internal_offset = readback.internal_offset;
         pvrgpu_counter_eventf("stream_output_readback",
            "generation=%llu buffer=%u cursor=%u bytes=%zu",
            (unsigned long long)readback.submission_generation,
            payload->output_buffer, target->internal_offset, resource->size);
      }
   }
   /*
                         ...
                         uint32_t *out_max_index)
{
   ...
   *out_index_count = retained_count;
                                ...
                                uint32_t *words)
{
   ...
   if (!abi || !words)
      return false;
   size_t available = 0;
   const uint8_t *bytes = abi->push_constant_count
      ? pvrgpu_constant_buffer_bytes(ctx, stage, 0, &available) : NULL;
   return pvrgpu_copy_push_constant_words(abi, map, bytes, available, words,
...
pvrgpu_capture_stage_uniform_buffers(
   ...
   const struct pvrgpu_compute_snapshot *graphics_buffer_snapshot,
   struct pvrgpu_array_primitive_draw *recorded, uint32_t *shared)
{
   ...
   if (!abi || (stage != MESA_SHADER_VERTEX && stage != MESA_SHADER_FRAGMENT &&
       ...
       !pvrgpu_uniform_buffers_disjoint_from_writable_snapshot(
          ctx->constant_buffers[stage], active_blocks,
          cb0_uniform_buffer_slot, graphics_buffer_snapshot) ||
   ...
   bool may_retry_payload, bool *terminal_failure)
{
   ...
   /* Every caller, including observable-draw fallbacks, must satisfy the
    * fixed four-target capsule before allocation, compilation or writes. */
   if (!pvrgpu_framebuffer_color_transport_is_bounded(ctx)) {
      pvrgpu_counter_eventf("draw_array_primitive_record_error",
   ...
   const bool current_graphics_buffers =
      ...
      (ctx->gs && ctx->gs->nir && ctx->gs->nir->info.num_ssbos) ||
      (ctx->tcs && ctx->tcs->nir && ctx->tcs->nir->info.num_ssbos) ||
      (ctx->tes && ctx->tes->nir && ctx->tes->nir->info.num_ssbos);
   const bool preceding_graphics_buffers = ctx->array_primitive_draw_count &&
      ctx->array_primitive_draws[ctx->array_primitive_draw_count - 1]
         ->graphics_buffers.resource_count;
   if ((ctx->num_stream_output_targets || current_images || preceding_images ||
        current_graphics_buffers || preceding_graphics_buffers) &&
       ctx->array_primitive_draw_count) {
   ...
   /* A VS can generate positions from system values/uniforms without a real
    * attribute. Only nonempty layouts need vertex buffers; the NIR input
    * mask and the packed system-value ABI are validated below. */
   if (!ctx->vertex_elements ||
       (ctx->vertex_elements->num_elements != 0 && ctx->num_vertex_buffers == 0) ||
       !ctx->vs || !ctx->fs ||
       !ctx->vs->nir || !ctx->fs->nir) {
      pvrgpu_counter_eventf("draw_array_primitive_record_error",
   ...
   const unsigned patch_vertices = has_tessellation ? ctx->patch_vertices : 0;
   if (has_tessellation && (!patch_vertices || patch_vertices > 32 || info->mode != MESA_PRIM_PATCHES)) {
      pvrgpu_counter_eventf("draw_array_primitive_record_error", "stage=tessellation reason=patch_input_extent");
      return false;
   }
   if ((has_tessellation ? draw->count / patch_vertices :
        pvrgpu_array_primitive_count(info->mode, draw->count)) == 0) {
   ...
   if (info->index_size != 0) {
      uint32_t max_index = 0;
      index_data =
         pvrgpu_copy_draw_indices(info, draw, assembled_count,
                                  &assembled_count, &max_index);
      if (!index_data) {
         pvrgpu_counter_eventf("draw_array_primitive_record_error",
      ...
      if (assembled_count == 0) {
         /* All POINTS indices were restart markers: no vertex fetch, GS/VS
          * invocation or generated primitive. Keep empty queries at zero. */
         pvrgpu_counter_eventf("draw_array_primitive_empty",
                               "mode=%u count=%u reason=point_restart_only",
   ...
   /*
    * Vertex system values use separate raw-integer VTXIN slots after the
    * real attributes, in the compiler's order: InstanceID, then VertexID.
    * Instancing itself is
   ...
   const bool reads_vertex_id =
      BITSET_TEST(ctx->vs->nir->info.system_values_read,
                  SYSTEM_VALUE_VERTEX_ID);
   const unsigned instance_id_attribute = attribute_count;
   const unsigned vertex_id_attribute = attribute_count + (reads_instance_id ? 1u : 0u);
   const unsigned packed_attribute_count =
      vertex_id_attribute + (reads_vertex_id ? 1u : 0u);
   if (packed_attribute_count > PVRGPU_PCO_MAX_VERTEX_ATTRIBUTES) {
      pvrgpu_counter_eventf("draw_array_primitive_record_error",
                            "stage=attributes reason=vertex_system_value_slots "
                            "attributes=%u",
                            attribute_count);
      free(index_data);
      return false;
   ...
   if (reads_instance_id) {
      attribute_components[instance_id_attribute] = 1;
      attribute_formats[instance_id_attribute] = PIPE_FORMAT_R32_UINT;
   }
   if (reads_vertex_id) {
      attribute_components[vertex_id_attribute] = 1;
      attribute_formats[vertex_id_attribute] = PIPE_FORMAT_R32_UINT;
   }
   ...
   if (info->index_size != 0 && transport_primitive_restart) {
      ...
      if (!pvrgpu_expand_instanced_connected_indices(info->mode, index_data, info->index_size,
             ...
             &command_index_count, &command_primitive_mode)) {
         free(index_data);
         return false;
      }
      free(index_data);
      index_data = (uint8_t *)expanded;
      index_data_size = (size_t)command_index_count * sizeof(uint32_t);
      command_index_size = 4;
   } else if (info->index_size != 0 && instance_count > 1) {
   ...
   if ((packed_words == 0 && !ctx->gs && !has_tessellation) ||
       (packed_words != 0 &&
    ...
    */
   uint32_t *interleaved = packed_words == 0 ? NULL :
      calloc((size_t)packed_vertex_count * packed_words, sizeof(uint32_t));
   if (packed_words != 0 && !interleaved) {
      free(index_data);
      return false;
   }
   for (unsigned instance = 0; instance < instance_count; ++instance) {
      for (unsigned v = 0; v < vertex_count; ++v) {
         ...
         if (reads_instance_id) {
             ...
             */
            interleaved[packed_base + attribute_offsets[instance_id_attribute]] =
               instance;
         }
         if (reads_vertex_id) {
            /* Preserve the API-visible ID, not the compacted index or the
             * instance-expanded stream offset. For indexed draws v_idx
             * already includes signed baseVertex; for arrays it includes
             * first. Repeated indices reuse this same original ID. */
            interleaved[packed_base + attribute_offsets[vertex_id_attribute]] =
               v_idx;
         }
      }
   }
   ...
   if (has_tessellation) {
      if (ctx->gs) {
         compiled = pvrgpu_pco_compile_tessellation_geometry_pipeline(
            ctx->pco_compiler, ctx->vs->nir, ctx->tcs->nir,
            ctx->tes->nir, ctx->gs->nir, ctx->fs->nir,
            attribute_formats, MAX2(1U, ctx->framebuffer.nr_cbufs),
            vertex_uniform_dwords,
            pvrgpu_stage_uniform_dwords(ctx, MESA_SHADER_TESS_CTRL),
            pvrgpu_stage_uniform_dwords(ctx, MESA_SHADER_TESS_EVAL),
            pvrgpu_stage_uniform_dwords(ctx, MESA_SHADER_GEOMETRY),
            fragment_uniform_dwords, attribute_count,
            fragment_texture_count, &tessellation, &geometry,
            error, sizeof(error));
      } else {
         compiled = pvrgpu_pco_compile_tessellation_pipeline(
            ctx->pco_compiler, ctx->vs->nir, ctx->tcs->nir,
            ctx->tes->nir, ctx->fs->nir, attribute_formats,
            MAX2(1U, ctx->framebuffer.nr_cbufs),
            vertex_uniform_dwords,
            pvrgpu_stage_uniform_dwords(ctx, MESA_SHADER_TESS_CTRL),
            pvrgpu_stage_uniform_dwords(ctx, MESA_SHADER_TESS_EVAL),
            fragment_uniform_dwords, attribute_count,
            fragment_texture_count, &tessellation, error, sizeof(error));
      }
      binary = tessellation.graphics;
      memset(&tessellation.graphics, 0, sizeof(tessellation.graphics));
   } else if (ctx->gs) {
      struct pvrgpu_pco_geometry_pipeline_binary pipeline = {0};
      compiled = pvrgpu_pco_compile_geometry_pipeline(
         ctx->pco_compiler, ctx->vs->nir, ctx->gs->nir, ctx->fs->nir,
         attribute_formats, MAX2(1U, ctx->framebuffer.nr_cbufs),
         vertex_uniform_dwords, pvrgpu_stage_uniform_dwords(ctx, MESA_SHADER_GEOMETRY),
         fragment_uniform_dwords, attribute_count, fragment_texture_count,
   ...
   if (decodable && ctx->gs) {
      decode_stage = "geometry";
      undecodable = &geometry.shader;
      decodable = pvrgpu_pco_binary_is_executable(
         PVRGPU_SYSTEMC_PCO_SHADER_STAGE_GEOMETRY,
         geometry.shader.data, geometry.shader.size, decode_error, sizeof(decode_error));
   }
   if (decodable && has_tessellation) {
      decode_stage = "tessellation_control";
      undecodable = &tessellation.control.shader;
      decodable = pvrgpu_pco_binary_is_executable(
         PVRGPU_SYSTEMC_PCO_SHADER_STAGE_TESS_CONTROL,
         undecodable->data, undecodable->size, decode_error, sizeof(decode_error));
   }
   if (decodable && has_tessellation) {
      decode_stage = "tessellation_evaluation";
      undecodable = &tessellation.evaluation.shader;
      decodable = pvrgpu_pco_binary_is_executable(
   ...
   command.first_vertex = 0;
   command.instance_count = 1;
   command.primitive_mode = (uint32_t)command_primitive_mode;
   command.indexed = command_index_size != 0 ? 1u : 0u;
   ...
   if (pvrgpu_framebuffer_has_mixed_color_formats(ctx) ||
      ...
      for (unsigned target = 0; target < command.render_target_count; ++target)
         command.color_attachment_formats[target] =
            pvrgpu_command_format_for_surface(ctx->framebuffer.cbufs[target].format);
   }
   ...
   uint32_t geometry_uniform_words[PVRGPU_COLOR_PRIMITIVE_UNIFORM_DWORDS] = {0};
   ...
   struct pvrgpu_systemc_graphics_shader_buffers graphics_buffer_payload = {0};
   struct pvrgpu_compute_snapshot graphics_buffer_snapshot = {0};
   struct pvrgpu_systemc_shader_buffer_resource
      graphics_buffer_resources[PVRGPU_SYSTEMC_MAX_SHADER_BUFFER_RESOURCES] = {{0}};
   struct pvrgpu_systemc_shader_buffer_binding
      graphics_buffer_bindings[PVRGPU_SYSTEMC_MAX_SHADER_BUFFER_BINDINGS] = {{0}};
   if (has_tessellation) {
      ...
      if (control->abi.shareds > PVRGPU_COLOR_PRIMITIVE_UNIFORM_DWORDS ||
          evaluation->abi.shareds > PVRGPU_COLOR_PRIMITIVE_UNIFORM_DWORDS ||
          !pvrgpu_copy_stage_uniform_words(ctx, MESA_SHADER_TESS_CTRL, &control->abi, &control->cb0_word_map, control_uniform_words) ||
          !pvrgpu_copy_stage_uniform_words(ctx, MESA_SHADER_TESS_EVAL, &evaluation->abi, &evaluation->cb0_word_map, evaluation_uniform_words)) {
         pvrgpu_pco_tessellation_pipeline_binary_finish(&tessellation);
         pvrgpu_pco_geometry_binary_finish(&geometry);
         pvrgpu_pco_graphics_binary_finish(&binary);
         free(interleaved);
         free(index_data);
         pvrgpu_counter_eventf("draw_array_primitive_record_error", "stage=tessellation reason=shared_transport");
         return false;
      }
      tessellation_payload.control_pco = control->data;
      tessellation_payload.control_pco_size = control->size;
      tessellation_payload.control_shared = control_uniform_words;
      tessellation_payload.control_shared_count = control->abi.shareds;
      tessellation_payload.evaluation_pco = evaluation->data;
      tessellation_payload.evaluation_pco_size = evaluation->size;
      tessellation_payload.evaluation_shared = evaluation_uniform_words;
      tessellation_payload.evaluation_shared_count = evaluation->abi.shareds;
      pvrgpu_copy_pco_stage_abi_to_systemc(&tessellation_payload.control_abi, &control->abi);
      pvrgpu_copy_pco_stage_abi_to_systemc(&tessellation_payload.evaluation_abi, &evaluation->abi);
      tessellation_payload.input_vertices = patch_vertices;
      tessellation_payload.output_vertices = tessellation.output_vertices;
      tessellation_payload.vertices_per_instance = assembled_count;
      tessellation_payload.input_stride_dwords = tessellation.input.stride_dwords;
      tessellation_payload.output_vertex_stride_dwords = tessellation.patch.vertex.stride_dwords;
      tessellation_payload.per_vertex_offset_dwords = tessellation.patch.per_vertex_offset_dwords;
      tessellation_payload.patch_stride_dwords = tessellation.patch.patch_stride_dwords;
      tessellation_payload.control_barrier_count = tessellation.control.barrier_count;
      /* Mesa spacing order is equal/odd/even; transport is equal/even/odd. */
      tessellation_payload.domain = tessellation.primitive_mode - TESS_PRIMITIVE_TRIANGLES;
      tessellation_payload.spacing = tessellation.spacing == TESS_SPACING_EQUAL ? 0 :
         tessellation.spacing == TESS_SPACING_FRACTIONAL_EVEN ? 1 : 2;
      tessellation_payload.clockwise = !tessellation.ccw;
      tessellation_payload.point_mode = tessellation.point_mode;
      command.tessellation = &tessellation_payload;
      pvrgpu_counter_eventf("tessellation_pco_compiled",
         "tcs_bytes=%zu tes_bytes=%zu tcs_temps=%u tes_temps=%u "
         "input_vertices=%u output_vertices=%u input_stride=%u output_stride=%u "
         "patch_offset=%u patch_stride=%u barriers=%u domain=%u spacing=%u clockwise=%u point_mode=%u",
         control->size, evaluation->size, control->abi.temps, evaluation->abi.temps,
         patch_vertices, tessellation.output_vertices, tessellation.input.stride_dwords,
         tessellation.patch.vertex.stride_dwords, tessellation.patch.per_vertex_offset_dwords,
         tessellation.patch.patch_stride_dwords, tessellation.control.barrier_count,
         tessellation_payload.domain, tessellation_payload.spacing,
         tessellation_payload.clockwise, tessellation_payload.point_mode);
   }
   if (ctx->gs) {
      if (geometry.shader.abi.shareds > PVRGPU_COLOR_PRIMITIVE_UNIFORM_DWORDS ||
          !pvrgpu_copy_stage_uniform_words(ctx, MESA_SHADER_GEOMETRY,
                                          &geometry.shader.abi, &geometry.shader.cb0_word_map, geometry_uniform_words)) {
         pvrgpu_pco_tessellation_pipeline_binary_finish(&tessellation);
         pvrgpu_pco_geometry_binary_finish(&geometry);
         pvrgpu_pco_graphics_binary_finish(&binary);
         free(interleaved);
      ...
      command.geometry_pco = geometry.shader.data;
      command.geometry_pco_size = geometry.shader.size;
      command.geometry_shared = geometry_uniform_words;
      command.geometry_shared_count = geometry.shader.abi.shareds;
      pvrgpu_copy_pco_stage_abi_to_command(&command.geometry_pco_abi, &geometry.shader.abi);
      command.geometry_input_primitive_vertices = geometry.abi.vertices_in;
      command.geometry_output_primitive = geometry.abi.output_primitive;
      command.geometry_max_vertices = geometry.abi.vertices_out;
      command.geometry_invocations = geometry.abi.invocations;
      command.geometry_input_stride_dwords = geometry.abi.input.stride_dwords;
      command.geometry_vertices_per_instance = assembled_count;
      command.geometry_layer_output_start = geometry.abi.output.count[VARYING_SLOT_LAYER]
         ? geometry.abi.output.start[VARYING_SLOT_LAYER] : 0;
      command.geometry_layer_output_count = geometry.abi.output.count[VARYING_SLOT_LAYER];
      command.geometry_primitive_id_output_start = geometry.abi.output.count[VARYING_SLOT_PRIMITIVE_ID]
         ? geometry.abi.output.start[VARYING_SLOT_PRIMITIVE_ID] : 0;
      command.geometry_primitive_id_output_count = geometry.abi.output.count[VARYING_SLOT_PRIMITIVE_ID];
   }
   ...
   if (vertex_shared_count > PVRGPU_COLOR_PRIMITIVE_UNIFORM_DWORDS ||
       ...
       !pvrgpu_copy_stage_uniform_words(ctx,
                                        MESA_SHADER_VERTEX,
                                        &binary.vertex.abi,
                                        &binary.vertex.cb0_word_map,
                                        vertex_uniform_words) ||
       !pvrgpu_copy_stage_uniform_words(ctx,
                                        MESA_SHADER_FRAGMENT,
                                        &binary.fragment.abi,
                                        &binary.fragment.cb0_word_map,
   ...
   /* Record the new driver-private mapping only when packing was required.
    * These are hashes of already validated/copied CPU words, not new GPU
    * reads or proof that this draw has completed. Legacy records are unchanged. */
   const struct pvrgpu_pco_owned_binary *mapped_stages[] = {
   ...
   for (unsigned stage = 0; stage < ARRAY_SIZE(mapped_stages); ++stage) {
      ...
      for (unsigned word = 0; word < map->count; ++word)
         for (unsigned byte = 0; byte < 4; ++byte) {
            map_hash ^= (map->source_words[word] >> (byte * 8)) & 255U;
            map_hash *= UINT64_C(1099511628211);
         }
      pvrgpu_counter_eventf("draw_array_primitive_uniform_word_map",
         "stage=%u source_dwords=%u count=%u shared_start=%u shareds=%u "
         "map_fnv1a64=%016llx packed_fnv1a64=%016llx",
         stage ? MESA_SHADER_FRAGMENT : MESA_SHADER_VERTEX,
         map->source_dwords, map->count, compiled->abi.push_constant_start,
         compiled->abi.shareds, (unsigned long long)map_hash,
         (unsigned long long)pvrgpu_pco_binary_fnv1a64(
            (const uint8_t *)(mapped_shared[stage] + compiled->abi.push_constant_start),
   ...
   if (!pvrgpu_capture_graphics_buffers(
          ctx, &binary, ctx->gs ? &geometry : NULL,
          has_tessellation ? &tessellation : NULL,
          vertex_uniform_words, fragment_uniform_words,
          geometry_uniform_words, control_uniform_words,
          evaluation_uniform_words, &graphics_buffer_payload,
          &graphics_buffer_snapshot, graphics_buffer_resources,
          graphics_buffer_bindings)) {
      pvrgpu_compute_snapshot_finish(&graphics_buffer_snapshot);
      pvrgpu_pco_tessellation_pipeline_binary_finish(&tessellation);
      pvrgpu_pco_geometry_binary_finish(&geometry);
      pvrgpu_pco_graphics_binary_finish(&binary);
      free(interleaved);
      free(index_data);
      pvrgpu_counter_eventf("draw_array_primitive_record_error",
                            "stage=graphics_buffers reason=capture");
      return false;
   }
   for (unsigned stage = 0; stage < ARRAY_SIZE(graphics_buffer_payload.storage);
        ++stage) {
      if (graphics_buffer_payload.storage[stage].descriptor_count) {
         command.graphics_buffers = &graphics_buffer_payload;
         break;
      }
   }
   command.sampled_texture_count_by_stage[
      PVRGPU_SYSTEMC_PCO_SHADER_STAGE_VERTEX] = vertex_texture_count;
   command.sampled_texture_count_by_stage[
      PVRGPU_SYSTEMC_PCO_SHADER_STAGE_FRAGMENT] = fragment_texture_count;
   command.sampled_texture_count_by_stage[
      PVRGPU_SYSTEMC_PCO_SHADER_STAGE_GEOMETRY] = geometry_texture_count;
   command.sampled_texture_count_by_stage[
      PVRGPU_SYSTEMC_PCO_SHADER_STAGE_TESS_CONTROL] = control_texture_count;
   command.sampled_texture_count_by_stage[
      PVRGPU_SYSTEMC_PCO_SHADER_STAGE_TESS_EVALUATION] =
         evaluation_texture_count;
   ...
   recorded->graphics_buffer_snapshot = graphics_buffer_snapshot;
   memset(&graphics_buffer_snapshot, 0, sizeof(graphics_buffer_snapshot));
   recorded->graphics_buffers = graphics_buffer_payload;
   memcpy(recorded->graphics_buffer_resources, graphics_buffer_resources,
          sizeof(graphics_buffer_resources));
   memcpy(recorded->graphics_buffer_bindings, graphics_buffer_bindings,
          sizeof(graphics_buffer_bindings));
   ...
   const bool compiled_ubo_prefix = !ctx->gs && !has_tessellation;
   if (!pvrgpu_capture_stage_uniform_buffers(
       ...
       (ctx->gs && !pvrgpu_capture_stage_uniform_buffers(
          ...
          &geometry.shader.abi, 0, &recorded->graphics_buffer_snapshot,
          recorded, geometry_uniform_words)) ||
   ...
   const struct {
      ...
      {MESA_SHADER_VERTEX, vertex_texture_count, vertex_uniform_words, vertex_shared_count, 0},
      {MESA_SHADER_FRAGMENT, fragment_texture_count, fragment_uniform_words, fragment_shared_count, 0},
      {MESA_SHADER_GEOMETRY, geometry_texture_count, geometry_uniform_words, geometry.shader.abi.shareds,
       PVRGPU_PCO_GEOMETRY_UBO_DESCRIPTOR_START},
      {MESA_SHADER_TESS_CTRL, control_texture_count, control_uniform_words, tessellation.control.shader.abi.shareds, 8},
      {MESA_SHADER_TESS_EVAL, evaluation_texture_count, evaluation_uniform_words, tessellation.evaluation.shader.abi.shareds, 4},
   };
   for (unsigned stage_index = 0; stage_index < ARRAY_SIZE(texture_stages); ++stage_index) {
    for (unsigned texture = 0; texture < texture_stages[stage_index].count; ++texture) {
      const unsigned captured_index = recorded->texture_count;
      const mesa_shader_stage stage = texture_stages[stage_index].stage;
      uint32_t *shared_words = texture_stages[stage_index].shared;
      const unsigned shared_count = texture_stages[stage_index].shared_count;
      ...
      if (!pvrgpu_capture_generic_sequence_texture(
             ...
             texture,
             texture,
             &recorded->textures[captured_index],
             &recorded->texture_bytes[captured_index],
             &texture_reason)) {
         pvrgpu_counter_eventf("draw_array_primitive_record_error",
       ...
       * texture * PVRGPU_PCO_TEXTURE_DESCRIPTOR_DWORDS in the stage's shared
       * file after the stage's system-input descriptors, then constants, so the
      ...
      const unsigned descriptor_start =
         texture_stages[stage_index].descriptor_start +
         texture * PVRGPU_PCO_TEXTURE_DESCRIPTOR_DWORDS;
      enum pipe_format descriptor_format = pvrgpu_effective_sampled_format(
         ctx->sampler_views[stage][texture]);
      if (strcmp(captured->format, "PIPE_FORMAT_R32G32B32A32_UINT") == 0)
         descriptor_format = PIPE_FORMAT_R32G32B32A32_UINT;
      else if (strcmp(captured->format, "PIPE_FORMAT_R32G32B32A32_SINT") == 0)
         descriptor_format = PIPE_FORMAT_R32G32B32A32_SINT;
      else if (strcmp(captured->format, "PIPE_FORMAT_R32G32B32A32_FLOAT") == 0)
         descriptor_format = PIPE_FORMAT_R32G32B32A32_FLOAT;
      if (descriptor_start + PVRGPU_PCO_TEXTURE_DESCRIPTOR_DWORDS >
             shared_count ||
      ...
      if (captured->texture_kind == 1U) {
         ...
         uint32_t *descriptor = &shared_words[descriptor_start];
      ...
      if (captured->texture_kind == 4U &&
          !pvrgpu_pco_set_cube_array_texture_layout(
             &shared_words[descriptor_start], descriptor_format, captured->layers)) {
         pvrgpu_counter_eventf("draw_array_primitive_record_error",
                               "stage=textures reason=cube_array_descriptor slot=%u",
                               texture);
         pvrgpu_array_primitive_draw_destroy(&recorded);
         return false;
      }
      ...
      if (captured->texture_kind == 4U) {
         ...
         pvrgpu_counter_eventf("draw_array_primitive_texture_snapshot",
            "stage=%u slot=%u view=%p texture_res=%p kind=4 format=%s "
            "width=%u height=%u faces=%u mips=%u first_layer=%u first_level=%u bytes=%zu",
            (unsigned)stage, texture, (const void *)view, (const void *)view->texture,
            captured->format, captured->mip[0].width, captured->mip[0].height,
            captured->layers, captured->mip_count,
            view->u.tex.first_layer, view->u.tex.first_level, captured->bytes_size);
         if (util_format_has_depth(util_format_description(view->format)))
            pvrgpu_counter_eventf("draw_array_primitive_cube_array_depth_view",
               "stage=%u slot=%u view=%p texture_res=%p source_format=%s "
               "storage_format=%s swizzle=%u,%u,%u,%u compare_mode=%u",
               (unsigned)stage, texture, (const void *)view, (const void *)view->texture,
               util_format_name(view->format), captured->format,
   ...
   memcpy(recorded->geometry_shared_words, geometry_uniform_words,
          sizeof(recorded->geometry_shared_words));
   command.geometry_shared = ctx->gs ? recorded->geometry_shared_words : NULL;
   if (has_tessellation) {
      memcpy(recorded->control_shared_words, control_uniform_words, sizeof(recorded->control_shared_words));
      memcpy(recorded->evaluation_shared_words, evaluation_uniform_words, sizeof(recorded->evaluation_shared_words));
   ...
   if (command.graphics_buffers) {
      recorded->graphics_buffers.resources =
         recorded->graphics_buffers.resource_count ?
            recorded->graphics_buffer_resources : NULL;
      recorded->graphics_buffers.bindings =
         recorded->graphics_buffers.binding_count ?
            recorded->graphics_buffer_bindings : NULL;
      command.graphics_buffers = &recorded->graphics_buffers;
   }
   ...
   /* The transport owns complete attachments, not a truncated prefix of a
    * larger depth image. Unequal attachment layer spans need a separate
    * per-attachment extent contract before they can be safely represented. */
   if (ctx->framebuffer.zsbuf.texture && ctx->framebuffer.nr_cbufs &&
       (unsigned)ctx->framebuffer.zsbuf.last_layer - ctx->framebuffer.zsbuf.first_layer + 1 !=
          MAX2(1, recorded->command.framebuffer_layers)) {
      pvrgpu_counter_eventf("draw_array_primitive_record_error",
                            "stage=attachment_load reason=unequal_layer_spans");
      pvrgpu_array_primitive_draw_destroy(&recorded);
      return false;
   }
   if (recorded->binary.explicit_varying_bindings) {
      recorded->command.varying_bindings = recorded->varying_bindings;
   ...
   if (ctx->array_primitive_draw_count == 0 &&
       (!pvrgpu_capture_initial_color_attachment(ctx, recorded) ||
        !pvrgpu_capture_initial_depth_attachment(ctx, recorded))) {
      pvrgpu_counter_eventf("draw_array_primitive_record_error",
   ...
   pvrgpu_retire_materialized_attachment_clears(
      ctx, false, recorded->command.initial_depth_attachment_bytes_size);
   ...
   pvrgpu_counter_eventf("draw_array_primitive_recorded",
                         ...
                         command.cull_face,
                         command.front_ccw);
   if (recorded->command.stream_output) {
      const uint64_t failures = ctx->query_statistics_failures;
      pvrgpu_flush_current_color_attachments(&ctx->base);
      if (ctx->query_statistics_failures != failures)
         return false;
   ...
   size_t detail_size)
{
   ...
   *reason = NULL;
   ...
   case MESA_PRIM_PATCHES:
      if (ctx->tcs && ctx->tes && ctx->patch_vertices > 0 && ctx->patch_vertices <= 32)
         break;
      *reason = "tessellation_patch_input_or_missing_stage";
      return false;
   case MESA_PRIM_LINES_ADJACENCY:
   case MESA_PRIM_LINE_STRIP_ADJACENCY:
   case MESA_PRIM_TRIANGLES_ADJACENCY:
   case MESA_PRIM_TRIANGLE_STRIP_ADJACENCY:
      if (ctx->gs)
         break;
      *reason = "adjacency_without_geometry";
      return false;
   default:
   ...
   if (ctx->tcs || ctx->tes) {
      if (!ctx->tcs || !ctx->tes || !ctx->tcs->nir || !ctx->tes->nir || info->mode != MESA_PRIM_PATCHES) {
         *reason = "tessellation_missing_stage_nir_or_patch_mode";
         return false;
      }
   }
   if (ctx->gs && !ctx->gs->nir) {
      *reason = "geometry_missing_nir";
      return false;
   }
   if (!ctx->vertex_elements ||
       (ctx->vertex_elements->num_elements != 0 && ctx->num_vertex_buffers == 0)) {
   ...
   if (fragment_texture_count > PVRGPU_PCO_MAX_TEXTURES ||
       ...
       control_texture_count > PVRGPU_PCO_MAX_TEXTURES ||
       evaluation_texture_count > PVRGPU_PCO_MAX_TEXTURES) {
      *reason = "too_many_textures";
      if (detail && detail_size) {
         snprintf(detail, detail_size, "fragment_textures=%u geometry_textures=%u limit=%u",
                  fragment_texture_count, geometry_texture_count,
                  (unsigned)PVRGPU_PCO_MAX_TEXTURES);
      }
      return false;
   }
   const mesa_shader_stage texture_stages[] = {MESA_SHADER_FRAGMENT, MESA_SHADER_GEOMETRY,
      MESA_SHADER_VERTEX, MESA_SHADER_TESS_CTRL, MESA_SHADER_TESS_EVAL};
   const unsigned texture_counts[] = {fragment_texture_count, geometry_texture_count,
      vertex_texture_count, control_texture_count, evaluation_texture_count};
   for (unsigned stage_index = 0; stage_index < ARRAY_SIZE(texture_stages); ++stage_index) {
    for (unsigned texture = 0; texture < texture_counts[stage_index]; ++texture) {
      const mesa_shader_stage stage = texture_stages[stage_index];
      const struct pipe_sampler_view *view =
         ctx->sampler_views[stage][texture];
      ...
      if ((stage == MESA_SHADER_TESS_CTRL || stage == MESA_SHADER_TESS_EVAL) &&
          view->texture->nr_samples > 1) {
         *reason = "tessellation_texture_requires_single_sample";
         return false;
      }
    }
   }
   ...
   const unsigned raster_samples = MAX2(1, ctx->framebuffer.samples);
   if (raster_samples > 16 || (raster_samples & (raster_samples - 1))) {
      *reason = "raster_sample_count";
      return false;
   }
   for (unsigned target = 0; target < ctx->framebuffer.nr_cbufs; ++target) {
      if (!ctx->framebuffer.cbufs[target].texture)
      ...
      const struct pipe_resource *texture = ctx->framebuffer.cbufs[target].texture;
      const unsigned storage_samples = MAX2(1, texture->nr_storage_samples ?
                                           texture->nr_storage_samples : texture->nr_samples);
      if (storage_samples != raster_samples) {
         *reason = "color_storage_sample_count";
         return false;
      }
   }
   if (ctx->framebuffer.zsbuf.texture) {
      const struct pipe_resource *texture = ctx->framebuffer.zsbuf.texture;
      const unsigned storage_samples = MAX2(1, texture->nr_storage_samples ?
                                           texture->nr_storage_samples : texture->nr_samples);
      if (storage_samples != raster_samples) {
         *reason = "depth_storage_sample_count";
         return false;
      }
   }
   if (ctx->framebuffer.width == 0 || ctx->framebuffer.height == 0) {
...
{
   struct pvrgpu_context *ctx = pvrgpu_context(pipe);
   const uint64_t failures_before_draw = ctx->query_statistics_failures;

   unsigned trace_draw_actions = 0;
   const bool rdc_replay = pvrgpu_trace_draw_actions(&trace_draw_actions) &&
                           trace_draw_actions != 0;
   /* A submitted command only closes a declared replay frame.  In a live
    * application, materialize that draw before recording the next one so its
    * attachment contents become the next command's LOAD input. */
   if (!rdc_replay &&
       (ctx->driver_draw_command_emitted ||
        pvrgpu_driver_draw_command_has_been_emitted())) {
      pvrgpu_flush_current_color_attachments(pipe);
   ...
   struct pipe_draw_info decoded_info;
   struct pipe_draw_start_count_bias decoded_draw;
   if (indirect) {
      const char *reason = "indirect_draw_shape";
      struct pvrgpu_indirect_draw decoded;
      const struct pvrgpu_resource *buffer =
         pvrgpu_resource(indirect->buffer);
      bool valid = info && draws && num_draws == 1;
      if (valid) {
         reason = "indirect_buffer_resource";
         valid = buffer && buffer->base.target == PIPE_BUFFER && buffer->data;
      }
      if (valid) {
         reason = "indirect_graphics_flush_incomplete";
         valid = !pvrgpu_context_has_incomplete_replay(ctx);
      }
      if (valid) {
         /* TF readback publishes model bytes to the resource while its target
          * references are alive; CS likewise completes snapshot writeback
          * before launch_grid returns. Finish pending graphics before taking
          * this snapshot, including when the buffer is not an attachment.
          */
         pvrgpu_flush_current_color_attachments(pipe);
         reason = "indirect_graphics_flush_failed";
         valid = ctx->query_statistics_failures == failures_before_draw;
      }
      if (valid) {
         const size_t available = MIN2(buffer->size, buffer->base.width0);
         valid = pvrgpu_decode_indirect_draw(buffer->data, available,
                    indirect->offset, indirect->stride, indirect->draw_count,
                    info->index_size, indirect->indirect_draw_count != NULL,
                    indirect->count_from_stream_output != NULL,
                    &decoded, &reason);
      }
      if (!valid) {
         pvrgpu_note_unsupported_draw(ctx, info, indirect, draws, num_draws,
                                      reason ? reason : "indirect_decode");
         return;
      }
      pvrgpu_counter_eventf("draw_indirect_decoded",
         "offset=%u stride=%u indexed=%u count=%u instances=%u first=%u "
         "base_vertex=%d base_instance=%u drawid=%u",
         indirect->offset, indirect->stride, info->index_size != 0,
         decoded.count, decoded.instance_count, decoded.first,
         decoded.base_vertex, decoded.base_instance, drawid_offset);
      if (pvrgpu_indirect_draw_is_empty(&decoded)) {
         pvrgpu_counter_event("draw_indirect_empty", "reason=zero_work");
         return;
      }
      decoded_info = *info;
      decoded_info.instance_count = decoded.instance_count;
      decoded_info.start_instance = decoded.base_instance;
      decoded_info.index_bounds_valid = false;
      decoded_draw.start = decoded.first;
      decoded_draw.count = decoded.count;
      decoded_draw.index_bias = decoded.base_vertex;
      info = &decoded_info;
      draws = &decoded_draw;
      indirect = NULL;
      /* All shape, shader, indexed and instancing validation below still
       * applies. Never feed Gallium's zero-count placeholder to the model.
       */
   }
   ...
   if (pvrgpu_draw_is_lowerable_array_primitive(ctx, info, indirect, draws,
       ...
       */
      if (rdc_replay &&
          (ctx->array_primitive_sequence_owns_command ||
           pvrgpu_driver_draw_command_has_been_emitted())) {
   ...
   /* Preserve the original gate/record refusal before a shape-specific
    * fallback can return. The native CPU-present guard must still refuse
    * substitution, but its safety reason alone does not explain why native
    * lowering was unavailable (for example, descriptor capacity or target). */
   pvrgpu_counter_eventf("draw_array_primitive_gate_declined",
                         "reason=%s detail=[%s]",
                         lowering_reason ? lowering_reason : "unknown",
                         lowering_detail);

   /* Preserve preceding generic draws before a different lowering path
    * submits this draw.  Otherwise that command claims the shared output
    * slot and prevents the recorded sequence from ever executing. */
   if (!rdc_replay && ctx->array_primitive_draw_count != 0)
      pvrgpu_flush_current_color_attachments(pipe);
...
#define COPY(field) out->field = in->field
   ...
   COPY(scratch_bytes);
   COPY(shared_memory_descriptor_start);
   COPY(shared_memory_descriptor_count);
   COPY(image_descriptor_start);
   COPY(image_descriptor_count);
                   ...
                   const struct pipe_grid_info *info)
{
   ...
   if (!compute->binary.data || compute->uniform_dwords != uniform_dwords) {
      ...
      pvrgpu_counter_eventf(
         ...
         "image_used=0x%x image_read=0x%x image_write=0x%x",
         compute->binary.size,
         (unsigned long long)pvrgpu_pco_binary_fnv1a64(
         ...
         compute->binary.abi.image_used_mask,
         compute->binary.abi.image_read_mask,
         compute->binary.abi.image_write_mask);
   }
   ...
   if (abi->stage.uniform_buffer_descriptor_count >
       ...
       abi->image_descriptor_count > PIPE_MAX_SHADER_IMAGES ||
   ...
   for (unsigned slot = 0; slot < abi->image_descriptor_count; ++slot) {
      const uint32_t bit = UINT32_C(1) << slot;
      if (!(abi->image_used_mask & bit)) continue;
      const uint32_t access = ((abi->image_read_mask & bit) ? 1U : 0U) |
                              ((abi->image_write_mask & bit) ? 2U : 0U);
      if (!pvrgpu_compute_snapshot_add_image(&snapshot, slot, access,
            &ctx->shader_images[MESA_SHADER_COMPUTE][slot], &reason))
         goto fail;
   }
   ...
   for (unsigned slot = 0; slot < abi->sampled_texture_count; ++slot) {
      ...
      if (captured->texture_kind == 4U &&
          !pvrgpu_pco_set_cube_array_texture_layout(
             descriptor, format, captured->layers)) {
   ...
   for (size_t i = 0; i < snapshot.image_count; ++i) {
      ...
      pvrgpu_counter_eventf("compute_image_snapshot",
         "slot=%u resource_index=%u resource_bytes=%zu offset=%llu bytes=%llu "
         "width=%u height=%u stride=%u format=%u access=0x%x",
         image->slot, image->resource_index,
         snapshot.resources[image->resource_index].bytes_size,
         (unsigned long long)image->offset, (unsigned long long)image->bytes_size,
         image->width, image->height, image->row_stride_bytes, image->format, image->access);
   }
   ...
   if (abi->image_write_mask) {
      /* All previous graphics attachments were materialized before capture.
       * A readback token for that generation must not replace model CS stores. */
      ctx->color_readback_pending_mask = 0;
